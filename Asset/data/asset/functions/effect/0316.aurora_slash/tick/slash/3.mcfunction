...
# 演出
    playsound entity.evoker.prepare_summon player @a ~ ~ ~ 0.9 1.9 0
    playsound minecraft:item.trident.throw player @a ~ ~ ~ 0.6 0.8

# 演出用Object召喚
    data modify storage api: Argument.ID set value 1128
    data modify storage api: Argument.FieldOverride.Count set value 3
...
# ダメージ
    ...
    execute as @e[type=#lib:living_without_player,tag=Enemy,tag=316.Target,distance=..4.5] run function api:damage/
