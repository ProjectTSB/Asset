...
# 周囲にモブがいないかつ、周囲にプレイヤーがいないなら付与
    execute if entity @s[tag=!236.Loneliness] unless entity @a[distance=0.1..30] unless entity @e[type=!player,tag=Friend,distance=..30] run function asset:effect/0236.solitariness/fullset/modifier/add
...
# 孤独じゃないなら解除
    execute if entity @s[tag=236.Loneliness] if entity @a[distance=0.1..30] run function asset:effect/0236.solitariness/fullset/modifier/remove
    execute if entity @s[tag=236.Loneliness] if entity @e[type=!player,tag=Friend,distance=..30] run function asset:effect/0236.solitariness/fullset/modifier/remove
