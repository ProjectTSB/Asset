#> asset:effect/0358.time_ticker_night/register
#
# Effectのデータを指定
#
# @within function asset:effect/0358.time_ticker_night/_/register

# ExtendsSafe (boolean) (default = false)
    # data modify storage asset:effect ExtendsSafe set value true
# ID (int)
    data modify storage asset:effect ID set value 358
# 名前 (TextComponentString)
    data modify storage asset:effect Name set value '{"text":"時刻み・月夜","color":"#467bb5"}'
# 説明文 (TextComponentString[])
    data modify storage asset:effect Description set value ['{"text":"魔法属性被ダメージが低下し、MP回復量が上昇する"}']
