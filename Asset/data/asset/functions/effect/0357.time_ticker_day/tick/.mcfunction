...
# エフェクトを付与した直後に、効果時間を0にするとendが呼び出されないバグが起きているため、一時的に効果時間を0にする際はmodifier/removeを呼び出す (修正後は要削除)
    execute if predicate lib:is_night unless predicate lib:dimension/is_end if data storage asset:context this{InTheEnd:false} run function asset:effect/0357.time_ticker_day/tick/night
    ...
    execute if predicate lib:is_night unless predicate lib:dimension/is_end run function asset:effect/0357.time_ticker_day/modifier/remove

# 今がエンド && 前tickにエンドにいない なら夜バフを付与
    execute if predicate lib:dimension/is_end if data storage asset:context this{InTheEnd:false} run function asset:effect/0357.time_ticker_day/tick/night
