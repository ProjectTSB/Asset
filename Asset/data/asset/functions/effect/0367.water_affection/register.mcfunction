#> asset:effect/0367.water_affection/register
#
# Effectのデータを指定
#
# @within function asset:effect/0367.water_affection/_/register

# ExtendsSafe (boolean) (default = false)
    # data modify storage asset:effect ExtendsSafe set value true
# ID (int)
    data modify storage asset:effect ID set value 367
# 名前 (TextComponentString)
    data modify storage asset:effect Name set value '{"text":"ウォーターアフェクション","color":"aqua"}'
# 説明文 (TextComponentString[])
    data modify storage asset:effect Description set value ['{"text":"水属性ダメージを受けるたびに、水耐性がアップする"}']
