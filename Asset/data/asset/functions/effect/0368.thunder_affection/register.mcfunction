#> asset:effect/0368.thunder_affection/register
#
# Effectのデータを指定
#
# @within function asset:effect/0368.thunder_affection/_/register

# ExtendsSafe (boolean) (default = false)
    # data modify storage asset:effect ExtendsSafe set value true
# ID (int)
    data modify storage asset:effect ID set value 368
# 名前 (TextComponentString)
    data modify storage asset:effect Name set value '{"text":"サンダーアフェクション","color":"gold"}'
# 説明文 (TextComponentString[])
    data modify storage asset:effect Description set value ['{"text":"雷属性ダメージを受けるたびに、雷耐性がアップする"}']
