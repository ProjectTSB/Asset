...
# チャージ段階に比例して演出とスタックを変える
    execute if score $Charge Temporary matches 26 run playsound block.respawn_anchor.charge player @a ~ ~ ~ 0.35 1.6
    execute if score $Charge Temporary matches 26 run particle dust_color_transition 0.000 1.000 0.886 1 0 0.235 1 ~ ~0.4 ~ 0.25 0.25 0.25 0 15 normal
    execute if score $Charge Temporary matches 26 run data modify storage asset:context Stack set value 2

# チャージ時音
    execute if score $Charge Temporary matches 4..26 anchored eyes positioned ^-0.35 ^-0.15 ^1.5 rotated ~ ~90 run function asset:effect/0320.fatalerror_charge/re-given/particle
