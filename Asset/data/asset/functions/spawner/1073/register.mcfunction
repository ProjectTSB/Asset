...
execute unless loaded 102 161 -545 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1073
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [102, 161, -545]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [369,423]
# 一度に召喚する数 (int)
...
# 最大召喚間隔 (int)
    data modify storage asset:spawner MaxSpawnDelay set value 300
# 近くのエンティティの最大数 (int)
