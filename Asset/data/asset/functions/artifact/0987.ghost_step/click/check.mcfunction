...
# 既に発動している場合、テレポートはここで発動してしまう(MP消費したくないし)
    ...
    execute as @e[type=area_effect_cloud,tag=RF.Bullet,distance=..150] if score @s RF.UserID = @p[tag=this] UserID at @s run function asset:artifact/0987.ghost_step/click/teleport
