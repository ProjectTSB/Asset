#> asset:artifact/0468.raging_demon/tick/check_target
#
...
# 周囲10M以内の敵全てのHPを取得
    execute as @e[tag=Enemy,tag=!Uninterferable,distance=..10] store result score @s Temporary run function asset:artifact/0468.raging_demon/tick/get_health
...
# 対象となるターゲットがいない場合、処理を実行しない
    execute unless entity @e[tag=Enemy,tag=D0.Target,tag=!Uninterferable,distance=..10] run tag @s remove CanUsed
