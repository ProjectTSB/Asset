...
# 既にバリアがあるなら、バリアが最大値かチェックし、最大値ならCanUsedを削除
    ...
    execute if data storage api: Return.Effect run function asset:artifact/1314.topaz_broach/trigger/2.check_condition/barrier_is_max
    execute if entity @s[tag=!CanUsed] run return fail

# 3.main.mcfunctionを実行する
    function asset:artifact/1314.topaz_broach/trigger/3.main
