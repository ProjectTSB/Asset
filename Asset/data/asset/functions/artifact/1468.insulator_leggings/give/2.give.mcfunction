#> asset:artifact/1468.insulator_leggings/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1468.insulator_leggings/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1468
# 神器のベースアイテム
...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text": "マナを部分的に遮断するレギンス。"}', '{"text": "属性の出力を大きく制限する代わりにダメージに強い。"}']
...
# 装備時ステータス補正 ({Type: string, Amount: double, Operation: string, StackReduction?: double}[]) (オプション)
    ...
    data modify storage asset:artifact Equipment.Modifiers append value {Type:"defense/base",Amount:0.10d,Operation:"multiply_base"}
