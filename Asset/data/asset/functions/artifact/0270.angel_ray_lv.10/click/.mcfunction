...
# 神器のトリガー処理
#
# @within function asset:artifact/alias/270/click/
...
# レイ処理
    # 処理
        execute positioned ~ ~1.4 ~ as @e[type=area_effect_cloud,tag=Init,distance=..0.01] run function asset:artifact/0270.angel_ray_lv.10/click/ray
    # 次tick以降用にschedule予約
        schedule function asset:artifact/0270.angel_ray_lv.10/click/recursive 1t
# 効果処理
    execute positioned ~ ~1.4 ~ run function asset:artifact/0270.angel_ray_lv.10/click/effect/
# リセット
