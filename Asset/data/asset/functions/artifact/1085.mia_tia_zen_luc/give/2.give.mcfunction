#> asset:artifact/1085.mia_tia_zen_luc/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1085.mia_tia_zen_luc/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1085
# 神器のベースアイテム
...
# 装備時ステータス補正 ({Type: string, Amount: double, Operation: string, StackReduction?: double}[]) (オプション)
    ...
    data modify storage asset:artifact Equipment.Modifiers append value {Type:"generic.movement_speed",Amount:-0.5d,Operation:"multiply_base"}
# 扱える神 (string[]) Wikiを参照
