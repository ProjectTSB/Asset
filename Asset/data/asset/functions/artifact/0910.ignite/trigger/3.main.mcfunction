...
# 殴った相手以外にタグを付ける
    execute positioned ^ ^ ^1.5 run tag @e[type=#lib:living_without_player,tag=!Victim,tag=!Uninterferable,distance=..3] add PA.SlashHit
    execute positioned ^ ^ ^2.5 run tag @e[type=#lib:living_without_player,tag=!Victim,tag=!Uninterferable,distance=..3] add PA.SlashHit
...
# 近接で殴った相手以外に500ダメ
    ...
    data modify storage api: Argument.ElementType set value "Fire"
    function api:damage/modifier
    execute as @e[type=#lib:living_without_player,tag=PA.SlashHit,distance=..10] run function api:damage/
    function api:damage/reset
...
# リセット
    tag @e[type=#lib:living_without_player,tag=PA.SlashHit,distance=..10] remove PA.SlashHit
