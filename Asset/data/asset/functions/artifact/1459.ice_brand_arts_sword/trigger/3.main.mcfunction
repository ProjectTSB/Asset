...
# 攻撃時演出
    ...
    execute as @e[type=#lib:living,tag=Victim,distance=..6] at @s run playsound block.glass.break master @a ~ ~ ~ 1.2 1.7
    execute as @e[type=#lib:living,tag=Victim,distance=..6] at @s run playsound entity.player.attack.sweep master @a ~ ~ ~ 1 0.8
