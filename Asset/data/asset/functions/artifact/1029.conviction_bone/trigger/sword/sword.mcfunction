...
# 寿命を減らす
    scoreboard players remove @s SL.FishTime 1
# クールを増やす
    scoreboard players add @s SL.CoolTime 1

# ダメージ
    #execute if score @s SL.CoolTime matches 4 run function

# 寿命が来たら発動するやつ。プレイヤーが発動する内容
    execute if score @s SL.FishTime matches 0 as @a if score @s UserID = @e[type=item_display,tag=SL.This,limit=1] SL.UserID run function asset:artifact/1029.conviction_bone/trigger/sword/sword_kill
