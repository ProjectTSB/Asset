#> asset:artifact/1118.time_samsara/tick/summon
#
#
#
# @within function asset:artifact/1118.time_samsara/tick/
...
# 拡散用マーカーが敵を見る
    execute as @e[type=marker,tag=SpreadMarker,distance=..30] at @s run tp @s ~ ~ ~ facing entity @e[type=#lib:living,tag=Enemy,distance=..30,sort=nearest,limit=1] eyes
...
# 召喚
    ...
    execute at @e[type=marker,tag=SpreadMarker,distance=..30] facing entity @e[type=#lib:living,tag=Enemy,distance=..30,sort=nearest,limit=1] eyes positioned ^ ^ ^-3 run function api:object/summon
