...
# 拡散用marker召喚
    execute as @e[type=#lib:living,distance=..30,limit=1] anchored eyes run summon marker ^ ^ ^ {Tags:["SpreadMarker"]}
...
# 召喚
    ...
    execute at @e[type=marker,tag=SpreadMarker,distance=..30] facing entity @e[type=#lib:living,tag=Enemy,distance=..30,limit=1] eyes positioned ^ ^ ^-3 run function api:object/summon
