#> asset:artifact/1174.sanguis/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1174.sanguis/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1174
# 神器のベースアイテム
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value ["80-110"]
...
# MP回復量
    data modify storage asset:artifact MPHealWhenHit set value 2.6
# 神器のクールダウン (int) (オプション)
