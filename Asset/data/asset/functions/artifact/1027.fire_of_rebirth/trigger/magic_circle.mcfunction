...
# 発動
    execute as @e[type=area_effect_cloud,tag=SH.Magic,scores={SH.CoolTime=0}] at @s positioned ~ ~1.5 ~ positioned ^ ^ ^2 run function asset:artifact/1027.fire_of_rebirth/trigger/shoot
...
# ループ
    execute if entity @e[type=area_effect_cloud,tag=SH.Magic,limit=1] run schedule function asset:artifact/1027.fire_of_rebirth/trigger/magic_circle 1t replace
