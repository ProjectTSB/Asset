...
# 地面に当たった場合ダメージ処理と強制終了
    execute unless block ~ ~ ~ #lib:no_collision run function asset:artifact/1033.thelema_of_blue_sea/attack_melee/entity/persuit/sword/hit
    execute unless block ~ ~ ~ #lib:no_collision run return 0

# 6回まで再帰
    execute if score $Count Temporary matches ..6 at @s run function asset:artifact/1033.thelema_of_blue_sea/attack_melee/entity/persuit/sword/recursive