...
# ターゲット指定 Victimと前方を対象にする
    tag @e[type=#lib:living,tag=Victim,tag=!Enemy.Boss,tag=!Immovable,tag=!Uninterferable,distance=..10] add Target
    execute as @e[type=#lib:living_without_player,tag=Enemy,tag=!Enemy.Boss,tag=!Uninterferable,distance=..5] at @p[tag=this] positioned ^ ^ ^-3 unless entity @s[distance=..3] run tag @s add Target
