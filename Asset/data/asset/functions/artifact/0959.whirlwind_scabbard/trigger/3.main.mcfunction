...
# ターゲット指定 Victimと前方を対象にする
    ...
    execute as @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,tag=!Enemy.Boss,distance=..5] at @p[tag=this] positioned ^ ^ ^-3 unless entity @s[distance=..3] run tag @s add Target
