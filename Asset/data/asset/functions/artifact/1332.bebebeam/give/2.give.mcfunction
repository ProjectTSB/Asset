#> asset:artifact/1332.bebebeam/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1332.bebebeam/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1332
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:carrot_on_a_stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"ビビビーム","color":"light_purple"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['[{"translate":"%1$s・%2$s・%3$s属性の順に、連続でビームを放つ","color":"white","with":[{"text":"火","color":"red"},{"text":"水","color":"aqua"},{"text":"雷","color":"yellow"}]}]','[{"text":"この神器をホットバーに追加で1本持つたびに、発射数が1本増加する","color":"white"}]']
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value "60-80x3"
