...
# 天使が近くにいたら使用不可
    execute if entity @s[tag=CanUsed] if entity @e[type=#lib:living_without_player,tag=Enemy.Boss,distance=..32] run tellraw @a {"text":"スーパードラゴンは怯えて呼び出しに応じなかった","color":"red"}
    execute if entity @s[tag=CanUsed] if entity @e[type=#lib:living_without_player,tag=Enemy.Boss,distance=..32] run tag @s remove CanUsed
