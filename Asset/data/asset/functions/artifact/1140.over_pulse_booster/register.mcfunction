#> asset:artifact/1140.over_pulse_booster/register
#
# 神器の作成部 ここでID等を定義する
#
# @within function asset:artifact/alias/1140/register
...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"水平方向の移動力が向上する。"}','{"text":"他部位を装備するとさらに移動力が向上する。"}','{"text":"─ 移動補助 開始。","color":"gray","italic":true}','{"text":"─ 本日ハ 転落事故防止週間デス。","color":"gray","italic":true}']
...
# 装備時補正 (Compound[]) (オプション)
    ...
    data modify storage asset:artifact Modifiers append value {Type:"generic.armor",Amount:4d,Operation:"add"}
