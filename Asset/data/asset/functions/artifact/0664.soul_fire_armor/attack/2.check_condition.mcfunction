...
# 前回使用から一定時間経過しているかチェックする
    execute store result score $IG.Temp Temporary run data get storage global Time
    scoreboard players operation $IG.Temp Temporary -= @s IG.LatestUseTick
    execute unless score $IG.Temp Temporary matches 20.. run tag @s remove CanUsed
    scoreboard players reset $IG.Temp Temporary
