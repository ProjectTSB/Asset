...
# チャージ演出
    ...
    execute if data storage api: Return.Effect.Field{IsFullCharge:false} run playsound ogg:mob.breeze.charge1 player @a ~ ~ ~ 0.8 0.7
    ...
    execute if data storage api: Return.Effect.Field{IsFullCharge: true} run playsound ogg:mob.breeze.charge1 player @a ~ ~ ~ 0.8 1.2
