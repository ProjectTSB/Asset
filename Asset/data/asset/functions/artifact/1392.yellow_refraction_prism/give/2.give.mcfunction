#> asset:artifact/1392.yellow_refraction_prism/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1392.yellow_refraction_prism/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1392
# 神器のベースアイテム
...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"攻撃対象3体への与ダメージ量の1%分MPを消費し"}','{"text":"各ダメージ量の10%分の雷属性ダメージを与える"}','{"text":"このダメージは自身のステータス補正の影響を受けない"}']
...
# 神器の発動条件 (TextComponentString) (オプション)
    data modify storage asset:artifact Condition set value '{"text":"雷属性以外で属性攻撃"}'
