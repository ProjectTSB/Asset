...
# 落下量を検知
    execute as @a[tag=25.EffectClear] run function asset:artifact/0077.swords_of_waterfall_climbing/attack_melee/3.3.falldistance
...
# 着地時の処理
    ...
    # 着地時に周囲にダメージ与える
        execute as @a[tag=25.EffectClear,tag=25.Landing] if data storage api: {OnGround:1b} at @s run function asset:artifact/0077.swords_of_waterfall_climbing/attack_melee/3.3.landing_attack
