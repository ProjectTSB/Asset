#> asset:artifact/1186.flame_boomerang/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1186.flame_boomerang/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1186
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:carrot_on_a_stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"フレイムブーメラン","color":"#FFAB03"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"前方へ炎のブーメランを投げる","color":"white"}','{"text":"ブーメランは同じ敵に行きと帰りで2回ヒットする","color":"white"}']
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value [145]
...
# MP回復量 (int)
    data modify storage asset:artifact MPHealWhenHit set value 18.5
# 神器のクールダウン (int) (オプション)
