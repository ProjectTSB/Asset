...
# 1000ダメージを与えてなければreturn
    ...
    execute unless entity @s[scores={FE.DamageSum=10000..}] run return run scoreboard players reset $DamageVal Temporary

# ダメージ処理
    execute if entity @s[scores={FE.DamageSum=10000..}] run function asset:artifact/0554.catastrophe/trigger/fullset/attack/3.damage_cooldown

# 与ダメージ1000毎(10倍で記録)にバフ
    execute if entity @s[scores={FE.DamageSum=10000..}] run function asset:artifact/0554.catastrophe/trigger/fullset/attack/5.buff
