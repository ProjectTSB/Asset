#> asset:artifact/1473.ninja_sandals/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1473.ninja_sandals/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1473
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:chainmail_boots"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text": "忍びの草鞋"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text": "一ツ、速ヤカニ動クコト。", "color": "gray"}']
...
# 装備時効果 ({ID: int, Visible: boolean}[]) (オプション)
    data modify storage asset:artifact Equipment.Effects set value [{ID:384,Visible:true,IsSetEffect:true}]
