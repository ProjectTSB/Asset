#> asset:artifact/0952.lunatic_rod/click/4.recursive
#
# 再帰レーザーを発射する
#
# @within function
...
# 定期的に光の輪を出す
    ...
    execute if score $Interval Temporary matches 5 run function asset:artifact/0952.lunatic_rod/click/vfx
...
# 再帰
    execute if entity @s[distance=..15] positioned ^ ^ ^1 if block ~ ~ ~ #lib:no_collision run function asset:artifact/0952.lunatic_rod/click/4.recursive
