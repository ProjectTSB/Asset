...
# 2個目のコマンドの最初にat @p[]があるのはdistanceの範囲を狭めるため
    execute at @p[tag=TL.TargetPlayer] rotated ~ 0 positioned ^ ^ ^-1 run summon marker ~ ~ ~ {Tags:["RotationMarker"]}
    execute at @p[tag=TL.TargetPlayer] as @e[type=marker,tag=RotationMarker,distance=..1,sort=nearest,limit=1] at @s run tp @s ~ ~ ~ facing entity @p[tag=TL.TargetPlayer]
