...
# 自身のIDが既にあれば自身のターゲット用Tagを削除
    $execute if data storage asset:temp TL.Temp.$(ID) run tag @s remove TL.TargetEntity

# データがなければ追加しておく
    $execute unless data storage asset:temp TL.Temp.$(ID) run data modify storage asset:temp TL.Temp.$(ID) set value 0b
