#> asset:artifact/1065.magic_bullet/click/loop/bullet/damage/
#
# ダメージ処理
#
...
# マーカーのデータを取り出し、既にヒット済みかをチェック
    ...
    execute as @e[type=#lib:living,tag=TL.TargetEntity,distance=..4] run function asset:artifact/1065.magic_bullet/click/loop/bullet/damage/check_target
...
# 7つ目の弾丸かつプレイヤーを狙っててかつ使用者に当たってるなら実行
    execute if entity @s[tag=TL.AimToPlayer,scores={TL.UseCount=7}] as @p[tag=Owner,tag=TL.TargetEntity] run function asset:artifact/1065.magic_bullet/click/loop/bullet/damage/hit_myself
