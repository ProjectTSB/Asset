...
# 演出
    execute at @e[type=#lib:living,tag=Victim,distance=..6] run playsound entity.player.attack.sweep player @a ~ ~ ~ 0.8 1 0
    execute at @e[type=#lib:living,tag=Victim,distance=..6] run playsound minecraft:block.beacon.activate player @a ~ ~ ~ 0.6 1.7
