...
# 攻撃対象3体を選定し、ダメージ量に比例したMPチェックなど
    function asset:artifact/1391.blue_refraction_prism/trigger/2.check_condition/check_damage
    execute if entity @s[tag=!CanUsed] run function asset:artifact/1391.blue_refraction_prism/trigger/2.check_condition/reset
    execute if entity @s[tag=!CanUsed] run return fail

# 3.main.mcfunctionを実行する
    function asset:artifact/1391.blue_refraction_prism/trigger/3.main

# リセット
    function asset:artifact/1391.blue_refraction_prism/trigger/2.check_condition/reset
