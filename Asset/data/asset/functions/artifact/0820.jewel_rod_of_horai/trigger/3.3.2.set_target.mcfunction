...
# 視線上かつブロック検知位置より手前にいるエンティティに候補タグ付け
    execute positioned ^ ^ ^6.4 as @e[type=#lib:living,tag=!Friend,distance=..6.4] positioned as @s anchored eyes positioned ^ ^ ^1000 facing entity @p[tag=this] eyes positioned ^ ^ ^1000 positioned ~-0.5 ~-0.5 ~-0.5 if entity @s[dx=0,dy=0,dz=0] run tag @s add MS.TargetCandidate
