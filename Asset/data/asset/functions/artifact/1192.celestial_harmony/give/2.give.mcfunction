#> asset:artifact/1192.celestial_harmony/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1192.celestial_harmony/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1192
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"セレスティアル・ハーモニー","color":"#5577FF"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"攻撃時、対象に星を降らせ追撃を行う"}','{"text":"追撃する星はMP割合が少ないほど増加する"}','{"text":"星の調べを奏でる天上の剣。","color":"gray"}','{"text":"剣の響きに応じて、夜空は星を返す。","color":"gray"}']
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value "600+150x?"
