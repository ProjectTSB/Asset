...
# モブに接触
    execute positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[type=#lib:living,type=!player,tag=!Uninterferable,dx=0] positioned ~0.5 ~0.5 ~0.5 run function asset:artifact/0210.terra_blade/click/3.5.terra_shot_hit
...
# 壁の衝突判定
    execute at @s unless block ~ ~ ~ #lib:no_collision run function asset:artifact/0210.terra_blade/click/terra_shot_break

# 再帰
    execute if entity @s[scores={5U.Recursion=1..,5U.Range=1..}] at @s run function asset:artifact/0210.terra_blade/click/3.3.terra_shot_main

# キル
    execute if entity @s[scores={5U.Range=0}] run function asset:artifact/0210.terra_blade/click/terra_shot_break
