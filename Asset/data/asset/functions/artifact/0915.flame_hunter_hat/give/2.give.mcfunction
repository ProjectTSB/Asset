...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['[{"text":"「森で緑を纏い、狩りをする者がいた。","color":"gray","italic":true}]','[{"text":"その者は獣を必要な数だけ狩り暮らしていた。」","color":"gray","italic":true}]']
...
# 装備時効果 ({ID: int, Visible: boolean}[]) (オプション)
    data modify storage asset:artifact Equipment.Effects set value [{ID:245,Visible:true,IsSetEffect:true}]
# 装備時ステータス補正 ({Type: string, Amount: double, Operation: string, StackReduction?: double}[]) (オプション)
    ...
    data modify storage asset:artifact Equipment.Modifiers append value {Type:"attack/fire",Amount:0.035d,Operation:"multiply_base"}
...
# カスタムNBT (NBTCompound) 追加で指定したいNBT (オプション)
    data modify storage asset:artifact CustomNBT set value {HideFlags:68,display:{color:10691106},Unbreakable:1b}
