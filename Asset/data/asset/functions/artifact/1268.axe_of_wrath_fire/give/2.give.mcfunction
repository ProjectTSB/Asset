#> asset:artifact/1268.axe_of_wrath_fire/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/1268.axe_of_wrath_fire/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 1268
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"狂焔の戦斧","color":"#CC1A1A"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"血の色か、焔の痕か","color":"gray"}','{"text":"それさえ忘れた者の斧","color":"gray"}']
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value [3100,3500]
...
# MP消費量 (int)
    data modify storage asset:artifact MPCost set value 170
# MP必要量 (int) (オプション)
