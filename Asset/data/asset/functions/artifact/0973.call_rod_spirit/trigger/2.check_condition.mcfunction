...
# 既に召喚済みなら使えない。ファミリアと違って召喚するのは一匹だが、合わせておくと都合がいいし、Lintを通すのも楽
    execute if entity @s[tag=CanUsed] at @e[type=armor_stand,tag=R1.Spirit] if score @s UserID = @e[type=armor_stand,tag=R1.Spirit,distance=..0.01,sort=nearest,limit=1] R1.UserID run tag @s add R1.CannotUse
