...
# タグ付与
    execute positioned ^ ^ ^1 run tag @e[type=#lib:living_without_player,tag=!Uninterferable,distance=..3] add RD.Hit
...
# ダメージ実行
    execute as @e[type=#lib:living_without_player,tag=RD.Hit,tag=!Uninterferable,distance=..16] run function api:damage/
...
# ノクバ耐性を考慮して吹っ飛ばす
    ...
    execute as @e[type=#lib:living_without_player,tag=RD.Hit,tag=!Uninterferable,distance=..16] at @s facing entity @p[tag=this] feet rotated ~ ~5 run function lib:motion/
    data remove storage lib: Argument

# リセット
    tag @e[type=#lib:living_without_player,tag=RD.Hit,distance=..16] remove RD.Hit
