...
# 加工済み用エラーメッセージ
    execute if data storage asset:context Items.offhand.tag.Hardened unless score @s XW.ErrorCooldown matches 1.. run tellraw @s [{"text":"既にその防具は加工済みのようだ…"}]
...
# 圧縮防具
    # それぞれチェック
        ...
        execute if entity @s[tag=!XW.SetArmor] unless score @s XW.ErrorCooldown matches 1.. run tellraw @s [{"text":"その神器を加工することは出来ないようだ…"}]
        execute if entity @s[tag=!XW.SetArmor] run return run tag @s remove CanUsed
    # 耐久チェック
        ...
        execute unless score $XW.Durability Temporary matches 4.. unless score @s XW.ErrorCooldown matches 1.. run tellraw @s [{"text":"圧縮された防具を加工するには足りないようだ…"}]
