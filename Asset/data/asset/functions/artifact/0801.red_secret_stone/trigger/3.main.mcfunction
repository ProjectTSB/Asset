...
# 所持数を基に回復量を計算 HealValue = 2n+3
    execute store result score $HealValue Temporary if data storage asset:context New.Items.hotbar[{tag:{TSB:{ID:801}}}]
    scoreboard players operation $HealValue Temporary *= $2 Const
    scoreboard players operation $HealValue Temporary += $3 Const

# 回復する
    execute store result storage lib: Argument.Heal int 1 run scoreboard players get $HealValue Temporary
