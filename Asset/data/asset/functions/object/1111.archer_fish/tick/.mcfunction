...
# ターゲットが居たらそいつを、居なければ近いやつをものすごい勢いでホーミングする
    execute if data storage asset:context this.TargetMobUUID positioned ^ ^ ^10 as @e[type=#lib:living,type=!player,distance=..10] if score @s MobUUID = $TargetMobUUID Temporary run tag @s add 1111.FishTarget
    execute unless data storage asset:context this.TargetMobUUID positioned ^ ^ ^10 as @e[type=#lib:living,type=!player,distance=..10,sort=nearest,limit=1] run tag @s add 1111.FishTarget
