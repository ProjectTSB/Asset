...
# 演出
    execute positioned ~-0.5 ~-0.5 ~-0.5 at @e[type=#lib:living_without_player,tag=1009.Target,dx=0] run particle dust 1.000 0.741 0.141 0.8 ~ ~1.2 ~ 0.5 0.5 0.5 0 50 normal
    execute positioned ~-0.5 ~-0.5 ~-0.5 at @e[type=#lib:living_without_player,tag=1009.Target,dx=0] run particle minecraft:falling_honey ~ ~1.2 ~ 0.5 0.5 0.5 0 20
...
# 鈍足をTickから秒に変換しマクロで付与
    ...
    execute as @e[type=#lib:living_without_player,tag=1009.Target,dx=0] run function asset:object/1140.arrow_of_honey_bee/pre_hit/slowness.m with storage asset:temp Args
