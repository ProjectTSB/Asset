...
# ターゲットがいればそいつを狙い、ターゲットがいなければ、前の近くの敵を狙う
    ...
    execute if data storage asset:context this.TargetMobUUID positioned ^ ^ ^15 unless entity @e[type=#lib:living_without_player,tag=1087.TargetCandidate,distance=..15] run data remove storage asset:context this.TargetMobUUID
    ...
    execute facing entity @e[type=#lib:living,tag=1087.TargetCandidate,distance=..30,sort=nearest,limit=1] eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-600 facing entity @s eyes positioned as @s run tp @s ~ ~ ~ ~ ~
