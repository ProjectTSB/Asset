...
# 敵を狙う
    execute if entity @e[type=#lib:living,tag=Enemy,distance=..30] at @e[tag=Enemy,distance=..30,sort=nearest,limit=1] positioned ~ ~1 ~ facing entity @s eyes positioned ^ ^ ^100 rotated as @s positioned ^ ^ ^-300 facing entity @s feet positioned as @s run tp @s ^ ^ ^ ~ ~
# 敵いないとプレイヤー見る
...
# Tick % 20 == 0 で発射
    ...
    execute if score $TickInterval Temporary matches 0 if entity @e[type=#lib:living,tag=Enemy,distance=..30] run function asset:object/1078.gun/tick/shot/
