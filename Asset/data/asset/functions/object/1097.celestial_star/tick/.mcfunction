...
# ターゲットが居たらそいつを、居なければ近いやつをものすごい勢いでホーミングする
    execute if data storage asset:context this.TargetMobUUID positioned ^ ^ ^10 as @e[type=#lib:living,type=!player,distance=..10] if score @s MobUUID = $TargetMobUUID Temporary run tag @s add 1097.CelestialTarget
    execute unless data storage asset:context this.TargetMobUUID positioned ^ ^ ^10 as @e[type=#lib:living,type=!player,distance=..10,sort=nearest,limit=1] run tag @s add 1097.CelestialTarget
...
# リセット
    ...
    tag @e[type=#lib:living,type=!player,tag=1097.CelestialTarget,distance=..20,limit=1] remove 1097.CelestialTarget
