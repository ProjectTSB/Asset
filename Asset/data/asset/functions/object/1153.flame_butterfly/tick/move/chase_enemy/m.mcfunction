...
# ターゲットが近くにいれば攻撃モードになる
    $execute unless data storage asset:context this{IsAttackMode:true} if data storage asset:context this.AttackCD{_:0} at @s positioned ~-1.2 ~-1.2 ~-1.2 if entity @e[type=#lib:living_without_player,tag=Enemy,scores={MobUUID=$(TargetMobUUID)},dx=1.4,dy=1.4,dz=1.4] run function asset:object/1153.flame_butterfly/tick/move/chase_enemy/set_attack_mode
