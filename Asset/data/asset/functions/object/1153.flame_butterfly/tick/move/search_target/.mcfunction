...
# 状況によっては毎tick大量の再帰をしてしまうため、数tickに1回のみ判定する
    execute store result storage asset:context this.SearchInterval int 0.9999999999 run data get storage asset:context this.SearchInterval
    execute unless data storage asset:context this{SearchInterval:0} run return fail
    data modify storage asset:context this.SearchInterval set value 3
