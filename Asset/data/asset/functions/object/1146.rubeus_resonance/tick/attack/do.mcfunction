...
# ターゲット
    tag @e[type=#lib:living,tag=!Uninterferable,distance=..2.8] add VU.Candidate
    execute positioned ^ ^ ^-1.4 run tag @e[type=#lib:living,tag=VU.Candidate,distance=..1.2] remove VU.Candidate
# ダメージ
# 候補のうち最寄りの１体に攻撃する
    ...
    execute as @e[type=#lib:living,tag=VU.Candidate,distance=..2.8,sort=nearest,limit=1] run function api:damage/
    function api:damage/reset
# リセット
    tag @e[type=#lib:living,tag=VU.Candidate,distance=..2.8] remove VU.Candidate
