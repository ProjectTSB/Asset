...
# 引数の設定
    # 与えるダメージ、待機時と開放時でダメージが違う
        execute if entity @s[tag=!2053.Release] run data modify storage api: Argument.Damage set value 10.0f
        execute if entity @s[tag=2053.Release] run data modify storage api: Argument.Damage set value 50.0f
    # 第一属性
