...
# ダメージ - プレイヤー
    ...
    data modify storage api: Argument.DeathMessage set value ['[{"translate": "%1$sはミサイルによって塵と化した。","with":[{"selector":"@s"}]}]']
...
# ダメージ - 敵
    ...
    data modify storage api: Argument.ElementType set value "Fire"
    execute as @p[tag=1054.Owner] run function api:damage/modifier
    execute positioned ~-3 ~-3 ~-3 as @e[type=#lib:living,type=!player,tag=Enemy,dx=5,dy=5,dz=5] run function api:damage/
    function api:damage/reset
