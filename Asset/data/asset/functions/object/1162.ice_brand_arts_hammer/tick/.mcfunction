...
# Tick加算
    ...
    execute as @s on passengers if data entity @s interaction run function asset:object/1162.ice_brand_arts_hammer/pick/

# 落下処理
    execute if score @s General.Object.Tick matches 10.. as @s[scores={Land=0}] at @s if block ~ ~-0.1 ~ air run function asset:object/1162.ice_brand_arts_hammer/tick/fall
