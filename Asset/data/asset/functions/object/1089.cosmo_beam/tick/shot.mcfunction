...
# 画面エフェクト
    execute positioned ~-8 ~-5 ~-8 as @a[dx=16,dy=100,dz=16] run title @s times 0 8 20
    execute positioned ~-8 ~-5 ~-8 as @a[dx=16,dy=100,dz=16] run title @s title {"text":"\uE010","font":"screen_effect","color":"#E8E8E8"}

# ダメージ設定


# 数をカウントして、ダメージを割る
    execute store result score $Count Temporary positioned ~-8 ~-5 ~-8 if entity @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,scores={MobID=0..},dx=16,dy=100,dz=16]
...
# ダメージ
    execute positioned ~-8 ~-5 ~-8 as @e[type=#lib:living,tag=Enemy,tag=!Enemy.Boss,tag=!Uninterferable,dx=16,dy=100,dz=16] run function api:damage/
# リセット
...
# もし天使がいるなら、天使には軽減したダメージを与える
    execute positioned ~-8 ~-5 ~-8 if entity @e[type=#lib:living,tag=Enemy.Boss,tag=!Uninterferable,dx=16,dy=100,dz=16] run function asset:object/1089.cosmo_beam/tick/angel_damage
