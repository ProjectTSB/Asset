#> asset:object/1162.frost_flake_arrow/baf/bloomdamage
#
#
...
# 元のdataの値をスコアボードに取得する (小数が切り捨てられないよう、100倍して取得)
    execute store result score $14D.value Temporary run data get storage asset:context this.BloomDamage 100
...
# ※取得時に100倍、倍率計算で100倍のスケールになっているため、戻す時に 0.0001 (1万分の1) を掛けて元のスケールに戻します
    execute store result storage asset:context this.BloomDamage double 0.0001 run scoreboard players get $14D.value Temporary
