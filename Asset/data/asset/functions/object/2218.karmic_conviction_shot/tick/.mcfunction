...
# 周囲に敵がいなければ、MovePerStepを0.5に戻す
    execute unless entity @a[gamemode=!spectator,distance=..20,sort=nearest,limit=1] run data modify storage asset:context this.MovePerStep set value 0.5
...
# 周囲に敵がいれば追尾 & MovePerStepを0.7に変更
    ...
    execute if entity @a[gamemode=!spectator,distance=..20,sort=nearest,limit=1] run data modify storage asset:context this.MovePerStep set value 0.7
