...
# Nは難易度値を示します
    ...
    scoreboard players add $DamageTemp Temporary 38
...
# ダメージ
    ...
    # 死亡ログ
        data modify storage api: Argument.DeathMessage set value '[{"translate": "%1$sは%2$sの裁きにより地獄へと落ちた","with":[{"selector":"@s"},{"nbt":"Return.AttackerName","storage":"lib:","interpret":true}]}]'
# 補正functionを実行
