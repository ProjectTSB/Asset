...
# TP
    execute if score @s General.Mob.Tick matches 0 run data modify storage lib: Argument.Bounds set value [[3d,3d],[0d,0d],[3d,3d]]
...
# 攻撃
    ...
    execute if score @s General.Mob.Tick matches 37 facing entity @p[gamemode=!spectator] feet rotated ~ 0 positioned ^ ^ ^4 run function asset:mob/0301.karmic/tick/common/tp
    execute if score @s General.Mob.Tick matches 37 run say slash
