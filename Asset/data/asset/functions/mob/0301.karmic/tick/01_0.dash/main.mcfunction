...
# 突っ込む
    execute if score @s General.Mob.Tick matches 33..80 facing entity @p[gamemode=!spectator,distance=..100] eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-400 facing entity @s eyes positioned as @s rotated ~ 0 positioned ^ ^ ^0.6 run function asset:mob/0301.karmic/tick/common/tp
# 近づいたらスコア設定
    execute if score @s General.Mob.Tick matches 33..80 if entity @p[gamemode=!spectator,distance=..3] run scoreboard players set @s General.Mob.Tick 80
# タグ
