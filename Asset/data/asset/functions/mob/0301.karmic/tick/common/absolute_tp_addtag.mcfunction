#> asset:mob/0301.karmic/tick/common/absolute_tp_addtag
#
#
#
...
# TPを確定させるためにタグを与える
    tag @s add 8D.TPConfirmed

# 自身をその場所まで持っていく(雑床埋まり対策をしない)
    tp @s ~ ~ ~ ~ ~
    execute at @s rotated ~ 0 run tp @e[type=item_display,tag=Uninterferable,tag=8D.AJ,tag=8D.AJLink,distance=..100,sort=nearest,limit=1] ~ ~ ~ ~ ~
