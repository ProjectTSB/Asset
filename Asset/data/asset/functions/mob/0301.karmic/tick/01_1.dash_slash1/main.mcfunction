...
# モーション変更
    ...
    execute if score @s General.Mob.Tick matches 0..7 rotated ~ 0 positioned ^ ^ ^0.2 run function asset:mob/0301.karmic/tick/common/tp
    execute if score @s General.Mob.Tick matches 8 rotated ~ 0 positioned ^ ^ ^3 run function asset:mob/0301.karmic/tick/common/tp
# スラッシュ
    execute if score @s General.Mob.Tick matches 8 run say slash
