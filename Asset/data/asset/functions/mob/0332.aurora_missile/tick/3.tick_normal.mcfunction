...
# 直進状態でブロックかプレイヤーにヒットで爆発する
    execute if entity @s[scores={98.Tick=20..}] unless block ^ ^ ^0.5 #lib:no_collision run function asset:mob/0332.aurora_missile/tick/5.hit
    execute if entity @s[scores={98.Tick=20..}] positioned ~-0.5 ~-0.5 ~-0.5 if entity @p[gamemode=!spectator,dx=0] run function asset:mob/0332.aurora_missile/tick/5.hit
