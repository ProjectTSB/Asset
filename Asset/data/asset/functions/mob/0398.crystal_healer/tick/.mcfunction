...
# ハードは待機猶予12秒
    execute if score @s B2.Tick matches 1 if predicate api:global_vars/difficulty/min/hard run scoreboard players set @s B2.Tick 61
...
# 演出
    # 30Tick 毎にクリスタルを回転させる
        ...
        execute if score $B2.Temp B2.Tick matches 10 on passengers on passengers run data merge entity @s {start_interpolation:-1,transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[0.75f,0.75f,0.75f]}}
        execute if score $B2.Temp B2.Tick matches 40 on passengers on passengers run data merge entity @s {start_interpolation:-1,transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,-0.4f,0f],scale:[0.75f,0.75f,0.75f]}}
