...
# スコア追加
    scoreboard players add $AZ.Temp Temporary 1
...
# そこがブロックで埋まってないか否かでループさせる
    execute at 0-0-0-0-0 if block ~ ~ ~ #lib:no_collision if score $AZ.Temp Temporary matches ..4 run function asset:mob/0393.labyria_first/ai/animation/2_0_sw_move_start/move/loop
