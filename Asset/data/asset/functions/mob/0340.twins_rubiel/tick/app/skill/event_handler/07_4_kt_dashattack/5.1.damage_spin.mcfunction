#> asset:mob/0340.twins_rubiel/tick/app/skill/event_handler/07_4_kt_dashattack/5.1.damage_spin
#
...
# ヒット判定
    execute positioned ^ ^ ^-1.5 run tag @a[distance=..2.5] add 9G.Temp.Target.Attack
    execute positioned ^ ^ ^ run tag @a[distance=..2.5] add 9G.Temp.Target.Attack
    execute positioned ^ ^ ^1.5 run tag @a[distance=..2.5] add 9G.Temp.Target.Attack
...
# ダメージクールダウン設定
    execute if entity @a[tag=9G.Temp.Target.Attack] run scoreboard players set @s 9G.DamageCooldown 4

# 気分を上昇
    execute if entity @a[tag=9G.Temp.Target.Attack] run scoreboard players add @s 9G.MoodPoint 2
