...
# ヒット判定
    tag @a[distance=..2.3] add 9F.Temp.Target.Attack
...
# 演出
    ...
    execute at @a[tag=9F.Temp.Target.Attack] positioned ~ ~1 ~ run particle block red_wool ~ ~1 ~ 0.1 0.2 0.1 0 5
    execute if entity @a[tag=9F.Temp.Target.Attack] run playsound ogg:entity.player.attack.knockback4 hostile @a ~ ~ ~ 2 1.3
