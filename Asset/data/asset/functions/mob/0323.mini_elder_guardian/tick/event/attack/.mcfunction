...
# プレイヤーのターゲティング
    execute if score @s General.Mob.Tick matches 0..39 facing entity @p[gamemode=!spectator,distance=..64] feet positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-500 facing entity @s eyes positioned as @s run tp @s ^ ^ ^ ~ ~
    execute if score @s General.Mob.Tick matches 40.. facing entity @p[gamemode=!spectator,distance=..64] feet positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-1500 facing entity @s eyes positioned as @s run tp @s ^ ^ ^ ~ ~

# 照射開始動作
    execute if score @s General.Mob.Tick matches 40 run function asset:mob/0323.mini_elder_guardian/tick/event/attack/start

# レーザー照射
    execute if score @s General.Mob.Tick matches 40..159 anchored eyes positioned ^ ^ ^ facing entity @p[gamemode=!spectator,distance=..32] eyes run function asset:mob/0323.mini_elder_guardian/tick/event/attack/laser/
