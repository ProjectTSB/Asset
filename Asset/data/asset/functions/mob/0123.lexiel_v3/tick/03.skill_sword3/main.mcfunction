...
# アニメーション開始
    execute if score @s 3F.Tick matches 0 facing entity @p[gamemode=!spectator,distance=..100] feet rotated ~ 0 run function asset:mob/0123.lexiel_v3/tick/common/tp
...
# ハードの場合、確率でジャンプ、特殊行動へ
    execute if score @s 3F.Tick matches 0 if predicate api:global_vars/difficulty/min/hard if predicate lib:random_pass_per/30 run scoreboard players set @s 3F.Tick 1000
...
# ハードモードの行動
    execute if score @s 3F.Tick matches 1000..1002 at @p[gamemode=!spectator,distance=..100] run function asset:mob/0123.lexiel_v3/tick/03.skill_sword3/portal_set
