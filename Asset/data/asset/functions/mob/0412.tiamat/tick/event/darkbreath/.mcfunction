...
# 攻撃
    ...
    # 攻撃位置決定
        execute if score @s BG.EventTimer matches 70 if predicate api:global_vars/difficulty/max/normal at @p[tag=BG.MainTarget] run function asset:mob/0412.tiamat/tick/event/darkbreath/summon_attack_pos
        execute if score @s BG.EventTimer matches 90 if predicate api:global_vars/difficulty/min/hard at @p[tag=BG.MainTarget] run function asset:mob/0412.tiamat/tick/event/darkbreath/summon_attack_pos
    # 攻撃
