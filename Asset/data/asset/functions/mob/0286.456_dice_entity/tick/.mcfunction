#> asset:mob/0286.456_dice_entity/tick/
#
# Mobのtick時の処理
#
# @within function asset:mob/alias/286/tick
...
# 数tick後
    ...
    execute if score @s CU.MoveTime matches 30 positioned ~ ~1.7 ~ unless entity @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,distance=..20,sort=nearest,limit=1] run scoreboard players add @s CU.MoveCount 1
    execute if score @s CU.MoveTime matches 30 positioned ~ ~1.7 ~ facing entity @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,distance=..20,sort=nearest,limit=1] eyes run function asset:mob/0286.456_dice_entity/tick/event/shoot_beam
