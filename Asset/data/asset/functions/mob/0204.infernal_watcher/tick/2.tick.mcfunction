#> asset:mob/0204.infernal_watcher/tick/2.tick
#
# Mobのtick時の処理
#
# @within function asset:mob/0204.infernal_watcher/tick/1.trigger
...
# サウンド
    ...
    execute if entity @s[scores={5O.MoveTime=100}] run function asset:mob/0204.infernal_watcher/tick/event/end_frenzy
...
# クールタイム中じゃないなら接触時に攻撃
    execute if entity @s[scores={5O.MoveTime=..59}] unless score @s 5O.AttackCT matches 1.. positioned ~-0.5 ~0.5 ~-0.5 if entity @p[tag=!PlayerShouldInvulnerable,dx=0] run function asset:mob/0204.infernal_watcher/tick/event/attack
    execute if entity @s[scores={5O.MoveTime=60..}] unless score @s 5O.AttackCT matches 1.. positioned ~-0.5 ~0.5 ~-0.5 if entity @p[tag=!PlayerShouldInvulnerable,dx=0] run function asset:mob/0204.infernal_watcher/tick/event/attack_strong
...
# 世界に存在しすぎた場合、消滅する
    ...
    execute if score @s 5O.LifeTime matches ..0 run function asset:mob/0204.infernal_watcher/tick/event/suicide
