...
# markerを召喚し、displayにそれを向かせる
    execute at @s rotated ~ 0 anchored eyes positioned ^0.2 ^-0.5 ^1.5 run summon marker ~ ~ ~ {Tags:["AC.RotationMarker"]}
    execute at @s anchored eyes positioned ^0.2 ^-0.5 ^18 run tp @e[type=item_display,tag=AC.LaserDisplay,distance=..50,limit=1] ~ ~ ~ ~ ~
