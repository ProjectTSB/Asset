...
# 既に落下中でなければNoAIを0bにして落下
    execute if entity @s[tag=!CT.Fall] if block ~ ~-0.1 ~ #lib:no_collision/ run function asset:mob/0461.nectar_plant/tick/fall/start

# 落下中は向きを固定する(NoAIでなくなるため、変な方向を向く可能性がある)
    execute if entity @s[tag=CT.Fall] run data modify entity @s Rotation set value [0f,0f]

# 落下中に地面に着いた場合はNoAIを1bにする
    execute if entity @s[tag=CT.Fall] if data entity @s {OnGround:1b} run function asset:mob/0461.nectar_plant/tick/fall/end
