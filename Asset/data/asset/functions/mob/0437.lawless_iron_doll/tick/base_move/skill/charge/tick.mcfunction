...
# 突進アニメ再生
    execute if score @s General.Mob.Tick matches 12 as @e[type=item_display,tag=C5.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:lawless_iron_doll/animations/run4/tween {to_frame: 0, duration: 5}

# 突進中のTick処理
    execute if score @s General.Mob.Tick matches 12..40 run function asset:mob/0437.lawless_iron_doll/tick/base_move/skill/charge/charge_tick

# ニュートラルポーズに戻る
    execute if score @s General.Mob.Tick matches 40 as @e[type=item_display,tag=C5.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:lawless_iron_doll/animations/neutral/tween {to_frame: 0, duration: 5}
