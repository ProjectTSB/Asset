...
# 近くのプレイヤーの方にそこそこの速度で向き直る
    execute if score @s General.Mob.Tick matches 0..10 facing entity @p[gamemode=!spectator,distance=..64] feet positioned ^ ^ ^-1 rotated as @s positioned ^ ^ ^-10 facing entity @s feet positioned as @s rotated ~ ~ run tp @s ~ ~ ~ ~ ~
...
# Blessless限定で、ジャンプ狩りの2発目を構える
    execute if predicate api:global_vars/difficulty/min/hard if score @s[scores={C5.Phase=2..}] General.Mob.Tick matches 27 as @e[type=item_display,tag=C5.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:lawless_iron_doll/animations/attack_melee_3_activate/tween {to_frame: 0, duration: 1}

# Blessless限定で、ジャンプ狩りを放つ
    execute if predicate api:global_vars/difficulty/min/hard if score @s[scores={C5.Phase=2..}] General.Mob.Tick matches 30 run function asset:mob/0437.lawless_iron_doll/tick/base_move/skill/horizontal_slash_shot/activate_second_slash
