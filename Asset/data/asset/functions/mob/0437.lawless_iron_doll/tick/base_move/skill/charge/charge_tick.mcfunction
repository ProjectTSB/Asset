...
# そこらのプレイヤーより上にいる場合、下にTP。下にブロックがあったら止まるよ。
    execute at @p[gamemode=!spectator,distance=..256] positioned ~-50 ~-2.5 ~-50 unless entity @s[dx=99,dy=-50,dz=99] at @s[tag=!AC.Opening,tag=!AC.InAction] if block ~ ~-0.25 ~ #lib:no_collision run tp @s ~ ~-0.25 ~
