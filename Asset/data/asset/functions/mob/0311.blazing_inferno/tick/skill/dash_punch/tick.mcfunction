...
# 手が震える
    execute if score @s General.Mob.Tick matches 35 as @e[type=item_display,tag=8N.ModelRoot,sort=nearest,limit=1] run function animated_java:blazing_inferno/animations/dash_charge/play
...
# ダッシュ終了
    execute if score @s General.Mob.Tick matches 80 run function asset:mob/0311.blazing_inferno/tick/skill/dash_punch/end

# 元の地点に戻る
    execute if score @s General.Mob.Tick matches 150 at @s run function asset:mob/0311.blazing_inferno/tick/base_move/return_to_rail

# リセット
    execute if score @s General.Mob.Tick matches 150 run function asset:mob/0311.blazing_inferno/tick/base_move/reset
