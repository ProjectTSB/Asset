...
# 攻撃
    ...
    # フィニッシュ、拡散弾
        ...
        # 攻撃を実行
            execute unless predicate api:global_vars/difficulty/min/hard if score @s General.Mob.Tick matches 50 run function asset:mob/0311.blazing_inferno/tick/skill/triple_fireball/shoot/vfx_heavy
            execute unless predicate api:global_vars/difficulty/min/hard if score @s General.Mob.Tick matches 50 run scoreboard players set @s 8N.AttackAmount 3
            execute if predicate api:global_vars/difficulty/min/hard if score @s General.Mob.Tick matches 50 run scoreboard players set @s 8N.AttackAmount 5
