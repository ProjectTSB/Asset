...
# rotatableDXYZで判定
    ...
    data modify storage lib: Args.selector set value "@a[gamemode=!spectator,tag=!PlayerShouldInvernerable,distance=..5]"
    execute positioned ^ ^0.5 ^0.75 run function lib:rotatable_dxyz/m with storage lib: Args

# DXYZのtag持ちがいたらダメージ処理実行
    execute if entity @p[gamemode=!spectator,tag=DXYZ,distance=..5] run function asset:mob/0175.queen_bee/tick/skill/tackle_formation/damage

# リセット
    tag @a[gamemode=!spectator,tag=DXYZ] remove DXYZ
