...
# SpawnMarkerを基準として、プレイヤーを見ながら回転する
    execute if score @s General.Mob.Tick matches 10..100 positioned as @e[type=marker,tag=4V.SpawnMarker,distance=..100,sort=nearest,limit=1] facing entity @s feet rotated ~1.5 0 positioned ^ ^ ^9 facing entity @p[tag=4V.Target] feet run function asset:mob/0175.queen_bee/tick/common/tp/
...
# 隊列を組む
    execute if score @s General.Mob.Tick matches 10..100 at @s run function asset:mob/0175.queen_bee/tick/skill/tackle_formation/set_formation

# 突進
    execute if score @s General.Mob.Tick matches 100.. at @s run function asset:mob/0175.queen_bee/tick/skill/tackle_formation/tackle

# 汎用ターゲット選択処理のリセット
    function asset:mob/0175.queen_bee/tick/skill/common/select_target/reset

# スキルのリセット
    execute if score @s General.Mob.Tick matches 130.. run function asset:mob/0175.queen_bee/tick/skill/reset
