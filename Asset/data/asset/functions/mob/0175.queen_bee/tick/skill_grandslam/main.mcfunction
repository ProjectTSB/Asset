...
# 一定距離落下したあと、地面にぶつかると爆発する
    execute if score @s General.Mob.Tick matches 75..85 at @s unless block ~ ~-1 ~ #lib:no_collision run function asset:mob/0175.queen_bee/tick/skill_grandslam/block_hit
    execute if score @s General.Mob.Tick matches 75..85 at @s unless block ~ ~-2 ~ #lib:no_collision run function asset:mob/0175.queen_bee/tick/skill_grandslam/block_hit
    execute if score @s General.Mob.Tick matches 75..85 at @s unless block ~ ~-3 ~ #lib:no_collision run function asset:mob/0175.queen_bee/tick/skill_grandslam/block_hit

# 戻る処理
    execute if score @s General.Mob.Tick matches 90 run function asset:mob/0175.queen_bee/tick/skill_grandslam/return
