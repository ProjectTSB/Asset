...
# 演出
    ...
    execute positioned ^ ^1.6 ^ rotated ~90 ~0 run function asset:mob/0327.eclael/tick/app.skill_events/12_latter_whip/5.2.particle_slash_0
    execute positioned ^ ^1.6 ^3 rotated ~90 ~0 run function asset:mob/0327.eclael/tick/app.skill_events/12_latter_whip/5.2.particle_slash_0
    execute positioned ^ ^1.6 ^6 rotated ~90 ~0 run function asset:mob/0327.eclael/tick/app.skill_events/12_latter_whip/5.2.particle_slash_0
...
# ライン斬撃
    ...
    execute if predicate api:global_vars/difficulty/min/hard as @e[type=area_effect_cloud,tag=93.Temp.AttackRotation] run tp @s ~ ~ ~ ~60 0
    ...
    execute if predicate api:global_vars/difficulty/min/hard as @e[type=area_effect_cloud,tag=93.Temp.AttackRotation] at @s run tp @s ~ ~ ~ ~60 0
    execute if predicate api:global_vars/difficulty/min/hard run data modify storage api: Argument.ID set value 2207
    execute if predicate api:global_vars/difficulty/min/hard store result storage api: Argument.FieldOverride.MobUUID int 1 run scoreboard players get @s MobUUID
...
# 演出
    ...
    data modify storage api: Argument.FieldOverride set value {Item:{id:"stick"},Color:16777088,Frames:[20502,20503,20504,20505],Scale:[18f,1f,20f],Transformation:{left_rotation:{axis:[0,0,1],angle:0.05f},right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f]}}
