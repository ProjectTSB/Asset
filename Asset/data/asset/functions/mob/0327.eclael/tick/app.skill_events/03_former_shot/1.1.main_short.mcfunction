...
# プレイヤーの方を向く
    ...
    execute if score @s 93.AnimationTimer matches 1..25 as @p[gamemode=!spectator,distance=..80] run function asset:mob/0327.eclael/tick/app.general/1.rotate
    execute if score @s 93.AnimationTimer matches 32..37 run tag @s add 93.Temp.Me
    execute if score @s 93.AnimationTimer matches 32..37 as @p[gamemode=!spectator,distance=..80] run function asset:mob/0327.eclael/tick/app.general/1.rotate
# 移動
