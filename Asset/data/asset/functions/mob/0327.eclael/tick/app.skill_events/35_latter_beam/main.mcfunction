...
# 開始
    ...
    # 中心点を挟んで反対側に移動
        ...
        execute if score @s 93.AnimationTimer matches 1 positioned as @e[type=marker,tag=93.Marker.SpawnPoint,distance=..80,sort=nearest,limit=1] facing entity @p[gamemode=!spectator,tag=93.Temp.Target] feet positioned as @s run tp @s ~ ~ ~ ~ 0
    ...
    # プレイヤーの方を向く
        ...
        execute if score @s 93.AnimationTimer matches 6..101 as @a[gamemode=!spectator,tag=93.Temp.Target,distance=..80,sort=nearest,limit=1] run function asset:mob/0327.eclael/tick/app.general/1.rotate
    # 演出
...
# 攻撃
    ...
    # 演出
        execute if score @s 93.AnimationTimer matches 127 run playsound item.armor.equip_leather hostile @a ~ ~ ~ 2 1
