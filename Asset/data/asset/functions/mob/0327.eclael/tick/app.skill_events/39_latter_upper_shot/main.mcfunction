...
# 開始
    ...
    # 中心点を挟んで反対側に移動
        ...
        execute if score @s 93.AnimationTimer matches 5 at @e[type=marker,tag=93.Marker.SpawnPoint,sort=nearest,limit=1] run tp @s ^ ^-2.3 ^-18 ~ 0
    # プレイヤーの方を向く
        execute if score @s 93.AnimationTimer matches 71..105 run tag @s add 93.Temp.Me
        execute if score @s 93.AnimationTimer matches 71..105 as @a[tag=!PlayerShouldInvulnerable,sort=nearest,limit=1] run function asset:mob/0327.eclael/tick/app.general/1.rotate
    # 演出
