...
# アニメーション再生
    ...
    # 中心点を挟んで反対側に移動
        ...
        execute if score @s 93.AnimationTimer matches 30 at @s as @e[type=marker,tag=93.Marker.SpawnPoint,sort=nearest,limit=1] positioned as @s run tp @s ~ ~ ~ ~0 0
        execute if score @s 93.AnimationTimer matches 30 at @e[type=marker,tag=93.Marker.SpawnPoint,sort=nearest,limit=1] run tp @s ^ ^-2.3 ^-3 ~ 0
    # 移動
