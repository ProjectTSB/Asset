...
# スコアが-1以下 && HurtTime中でない ならプレイヤーを追尾する
    execute if score @s General.Mob.Tick matches ..-1 if data storage asset:context this.HurtTime{_:0} run function asset:mob/0186.ferocious_bee/tick/turning
...
# 突進直前に速度設定
    execute if score @s General.Mob.Tick matches 20 run data modify entity @s Attributes[{Name:"minecraft:generic.movement_speed"}].Base set from storage asset:context this.MoveSpeed.Charge
# 20以上で突進
    execute if score @s General.Mob.Tick matches 20.. run function asset:mob/0186.ferocious_bee/tick/move
# 突進時に攻撃
    execute if score @s General.Mob.Tick matches 20.. positioned ~-0.5 ~0.5 ~-0.5 if entity @p[tag=!PlayerShouldInvulnerable,dx=0] run function asset:mob/0186.ferocious_bee/tick/damage/
...
# 演出
    execute if score @s General.Mob.Tick matches 20 run playsound entity.bee.hurt hostile @a ~ ~ ~ 1 1
    execute if score @s General.Mob.Tick matches 20.. run particle cloud ^ ^0.7 ^-0.8 0.25 0.25 0.25 0 0
