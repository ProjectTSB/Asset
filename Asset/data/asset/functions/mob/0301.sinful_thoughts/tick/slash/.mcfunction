#> asset:mob/0301.sinful_thoughts/tick/slash/
#
#
#
...
# 最初に実行
    execute if score @s 8D.Tick matches 0 run function asset:mob/0301.sinful_thoughts/tick/slash/first
...
# タグをつけて一度しか攻撃が当たらないようにする
    ...
    execute if score @s 8D.Tick matches 21 run function asset:mob/0301.sinful_thoughts/tick/slash/damage
...
# 確率でそのままジャンプする(ただしブロックに埋まっていない場合)
    execute if score @s 8D.Tick matches 26 if block ~ ~ ~ #lib:no_collision if predicate lib:random_pass_per/50 run function asset:mob/0301.sinful_thoughts/tick/slash/transition_jump
# リセット
