...
# フレアダイブ
    ...
    # 攻撃
        ...
        execute if score @s BF.EventTimer matches 462 if predicate api:global_vars/difficulty/min/hard at @s positioned ~ ~0.1 ~ run function asset:mob/0411.behemoth/tick/event/terzetto_purgatorio/dive_prediction
...
# 移動
    # アニメーション再生
        execute if score @s BF.EventTimer matches 733 as @e[type=item_display,tag=BF.ModelRoot,sort=nearest,limit=1] run function asset:mob/0411.behemoth/tick/animated_java/play/6_1_flymove_end
    # 中心点の正面方向に移動
