#> asset:mob/0380.haruclaire_v3/tick/event/ice_siege/summon_attack_pos
#
# アイスシージ
#
...
# 一定確率で逆向き
    execute if predicate lib:random_pass_per/50 run return run function asset:mob/0380.haruclaire_v3/tick/event/ice_siege/summon_attack_pos_inverse
...
# ハードではさらに設置
    ...
    execute as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition.Hard] at @s run function asset:mob/0380.haruclaire_v3/tick/util/setup_to_floor
    execute as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition.Hard] at @s facing entity @p[tag=AK.MainTarget] feet run tp @s ~ ~ ~ ~-45 0
