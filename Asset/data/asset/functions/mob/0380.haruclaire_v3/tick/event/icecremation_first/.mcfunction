...
# 2
    # アニメーション再生
        execute if score @s AK.EventTimer matches 184 as @e[type=item_display,tag=AK.ModelRoot,sort=nearest,limit=1] run function asset:mob/0380.haruclaire_v3/tick/animated_java/play/26_1_second_ice_cremation_short
    # 攻撃
        # 召喚位置決定
        ...
        # 召喚
            execute if score @s AK.EventTimer matches 213 as @e[type=area_effect_cloud,tag=AK.IceCrepation.Pos.A,limit=1] at @s run function asset:mob/0380.haruclaire_v3/tick/event/icecremation_first/attack_1
            execute if score @s AK.EventTimer matches 213 as @e[type=area_effect_cloud,tag=AK.IceCrepation.Pos.B,limit=1] at @s run function asset:mob/0380.haruclaire_v3/tick/event/icecremation_first/attack_2
