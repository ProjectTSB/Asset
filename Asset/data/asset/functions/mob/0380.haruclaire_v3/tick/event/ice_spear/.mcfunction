...
# 発射
    ...
    # 攻撃地点設置
        ...
        execute if score @s AK.EventTimer matches 105 if predicate api:global_vars/difficulty/normal as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=random,limit=1] run tag @s add AK.IceSpear.Spread
        execute if score @s AK.EventTimer matches 105 if predicate api:global_vars/difficulty/min/hard as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=random,limit=2] run tag @s add AK.IceSpear.Spread
    ...
    # 攻撃
        ...
        execute if score @s AK.EventTimer matches 54..57 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=arbitrary,limit=1] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack
        execute if score @s AK.EventTimer matches 73..76 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=arbitrary,limit=1] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack
        execute if score @s AK.EventTimer matches 87..90 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=arbitrary,limit=1] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack
        execute if score @s AK.EventTimer matches 96..99 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=arbitrary,limit=1] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack
        execute if score @s AK.EventTimer matches 115..150 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,tag=!AK.IceSpear.Spread,sort=arbitrary,limit=1] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack
        execute if score @s AK.EventTimer matches 120 at @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,tag=AK.IceSpear.Spread] run function asset:mob/0380.haruclaire_v3/tick/event/ice_spear/attack_spread
