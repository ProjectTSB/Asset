...
# 発射
    # アニメーション再生
        execute if score @s AK.EventTimer matches 1 as @e[type=item_display,tag=AK.ModelRoot,sort=nearest,limit=1] run function asset:mob/0380.haruclaire_v3/tick/animated_java/play/22_0_second_sword
    # 移動
...
# 攻撃
    # 攻撃位置決定
        ...
        execute if score @s AK.EventTimer matches 65 run tag @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition] add AK.Temp.Start
    ...
    # 1回目
        execute if score @s AK.EventTimer matches 65 as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,tag=AK.Temp.Start] at @s run function asset:mob/0380.haruclaire_v3/tick/event/icicle_blade/prediction.m {Rotate:90}
