...
# 地面に合わせる
    ...
    execute as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition] positioned as @s run tp @s ~ ~0.6 ~ ~ 0
...
# 予告円表示
    ...
    execute positioned as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition] positioned ~ ~ ~ run function api:object/summon
