...
# 攻撃
    ...
    # 召喚位置決定
        execute if score @s AK.EventTimer matches 1 at @e[type=marker,tag=AK.CenterPosition] run function asset:mob/0380.haruclaire_v3/tick/event/icecremation_four_circle/set_position
    # 召喚
        execute if score @s AK.EventTimer matches 30 as @e[type=area_effect_cloud,tag=AK.IceCrepation.Pos] at @s run function asset:mob/0380.haruclaire_v3/tick/event/icecremation_four_circle/attack
