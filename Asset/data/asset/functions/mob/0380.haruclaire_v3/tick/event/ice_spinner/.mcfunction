...
# 発射
    ...
    # 攻撃
        ...
        execute if score @s AK.EventTimer matches 40 as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=random] at @s run function asset:mob/0380.haruclaire_v3/tick/event/ice_spinner/attack_start
        execute if score @s AK.EventTimer matches 60 as @e[type=area_effect_cloud,tag=AK.Temp.AttackPosition,sort=random] at @s run function asset:mob/0380.haruclaire_v3/tick/event/ice_spinner/attack_start_long
