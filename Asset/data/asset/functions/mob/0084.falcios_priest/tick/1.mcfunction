...
# 技選択
    execute if predicate api:global_vars/difficulty/max/normal if entity @s[scores={2C.Tick=80..}] run function asset:mob/0084.falcios_priest/tick/2.select_skill
...
# ハード以上の場合二つのスキルを同時に使う
    execute if predicate api:global_vars/difficulty/min/hard if entity @s[scores={2C.Tick=80..}] run function asset:mob/0084.falcios_priest/tick/3.heal
    execute if predicate api:global_vars/difficulty/min/hard if entity @s[scores={2C.Tick=80..}] run function asset:mob/0084.falcios_priest/tick/4.shoot
