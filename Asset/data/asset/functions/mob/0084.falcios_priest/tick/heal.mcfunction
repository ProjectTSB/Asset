...
# 演出
    ...
    execute as @e[type=#lib:living_without_player,tag=Enemy,tag=!HasHealAbility,tag=!Uninterferable,distance=..10,sort=nearest,limit=5] at @s run particle heart ~ ~1.2 ~ 0.4 0.4 0.4 0 20 normal @a
...
# 難易度値によって回復量が変動
    ...
    execute as @e[type=#lib:living_without_player,tag=Enemy,tag=!HasHealAbility,tag=!Uninterferable,distance=..10,sort=nearest,limit=5] run function api:heal/
