...
# 演出
    ...
    execute as @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,tag=!NotTarget,distance=..10,sort=nearest,limit=5] at @s run particle heart ~ ~1.2 ~ 0.4 0.4 0.4 0 20 normal @a
...
# 難易度値によって回復量が変動 150N
    ...
    execute as @e[type=#lib:living,tag=Enemy,tag=!Uninterferable,tag=!NotTarget,distance=..10,sort=nearest,limit=5] run function api:heal/
...
# ハード以上では魔法発射側でリセットするためこちらではリセットしない
    execute unless predicate api:global_vars/difficulty/min/hard if entity @s[scores={General.Mob.Tick=0..}] run function asset:mob/0084.falcios_priest/tick/reset
