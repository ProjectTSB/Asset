...
## 移動射撃
# 双子に指示
    execute if score @s 9E.SkillTimer matches 3 as @e[type=wither_skeleton,tag=9F.Root,tag=9F.Target] at @s run function asset:mob/0339.twins_sapphiel/app/call_from_root/skill/01_crossfire/1.start
    execute if score @s 9E.SkillTimer matches 3 as @e[type=wither_skeleton,tag=9G.Root,tag=9G.Target] at @s run function asset:mob/0340.twins_rubiel/app/call_from_root/skill/01_crossfire/1.start
# 待機
...
## 追撃
# ルビィから通知が来た場合、サフィに指示
    execute if entity @s[tag=9E.Skill.Crossfire.Ruby.Hit] if score @s 9E.SkillTimer matches 4 as @e[type=wither_skeleton,tag=9F.Root,tag=9F.Target] at @s run function asset:mob/0339.twins_sapphiel/app/call_from_root/skill/01_crossfire/2.start_pursuit
    execute if entity @s[tag=9E.Skill.Crossfire.Ruby.Hit] if score @s 9E.SkillTimer matches 4 run tag @s remove 9E.Skill.Crossfire.Ruby.Hit

# 終了
    execute if score @s 9E.SkillTimer matches 5.. run function asset:mob/0338.corundum_twins/tick/app/skill/event_handler/01_crossfire/2.end
