#> asset:mob/0338.corundum_twins/tick/app/skill/event_handler/00_goalong/1.main
#
# スキルイベントハンドラ 集合
# 渋谷駅ハチ公前で集合する
#
...
# 集合位置の決定
    execute if score @s 9E.SkillTimer matches 1 at @a[tag=!PlayerShouldInvulnerable,sort=random,limit=1] rotated ~ 0 run summon area_effect_cloud ^ ^1 ^5 {CustomNameVisible:0b,Particle:"block air",Duration:17,Tags:["Object","9E.Temp.Target.Aec.0"]}
...
# 終了
    execute if score @s 9E.SkillTimer matches 3.. run function asset:mob/0338.corundum_twins/tick/app/skill/event_handler/00_goalong/2.end
