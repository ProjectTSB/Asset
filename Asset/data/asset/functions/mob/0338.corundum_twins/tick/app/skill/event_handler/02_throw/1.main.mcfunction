...
# 集合位置の決定
    execute if score @s 9E.SkillTimer matches 1 at @a[tag=!PlayerShouldInvulnerable,sort=random,limit=1] rotated ~ 0 run summon area_effect_cloud ^ ^1 ^8 {CustomNameVisible:0b,Particle:"block air",Duration:2,Tags:["Object","9E.Temp.Target.Aec.0"]}
...
# 移動位置計算
    ...
    execute if score @s 9E.SkillTimer matches 24..52 at @e[type=wither_skeleton,tag=9F.Root,tag=9F.Target,limit=1] run tp @e[type=area_effect_cloud,tag=9G.Temp.Target.Aec.0] ^ ^0.2 ^1 ~180 ~
# 攻撃位置決定
