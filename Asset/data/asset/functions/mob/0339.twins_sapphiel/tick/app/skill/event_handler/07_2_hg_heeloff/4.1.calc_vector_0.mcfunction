#> asset:mob/0339.twins_sapphiel/tick/app/skill/event_handler/07_2_hg_heeloff/4.1.calc_vector_0
#
# アニメーションのイベントハンドラ Hgかかと落とし 移動ベクトル計算
#
...
# vector/tickを計算
    scoreboard players set @s Temporary 7
    scoreboard players operation @s 9F.MoveVector.X /= @s Temporary
    scoreboard players operation @s 9F.MoveVector.Y /= @s Temporary
    scoreboard players operation @s 9F.MoveVector.Z /= @s Temporary
