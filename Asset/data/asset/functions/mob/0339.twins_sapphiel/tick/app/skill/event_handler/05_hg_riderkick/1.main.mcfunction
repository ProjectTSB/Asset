...
# 移動
    ...
    execute if score @s 9F.AnimationTimer matches 1..10 at @s positioned ^ ^0.25 ^-0.2 run function asset:mob/0339.twins_sapphiel/app/general/3.teleport
    ...
    # ベクトル計算移動
        execute if score @s 9F.AnimationTimer matches 19 at @a[tag=!PlayerShouldInvulnerable,sort=nearest,limit=1] run summon area_effect_cloud ^ ^0.1 ^-1 {CustomNameVisible:0b,Particle:"block air",Duration:5,Tags:["Object","9F.Temp.Target.Aec.0"]}
        execute if score @s 9F.AnimationTimer matches 20 run function asset:mob/0339.twins_sapphiel/tick/app/skill/event_handler/05_hg_riderkick/4.calc_vector
...
# 演出
    ...
    execute if score @s 9F.AnimationTimer matches 20 run playsound entity.wither.shoot hostile @a ~ ~ ~ 1 1.8
