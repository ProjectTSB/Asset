...
# 動くやつ
    execute if score @s AI.Tick matches 0 if entity @s[tag=!AI.Opening,tag=!AI.Skill90Used] unless entity @p[distance=..6] run tag @s add AI.Skill90



# その後発動するスキル
# プレイヤーが周囲にいたらスキル選択
    execute if score @s AI.Tick matches 0 if entity @s[tag=!AI.Opening,tag=!AI.Skill90Used,tag=!AI.Skill90] if entity @p[gamemode=!spectator,distance=..100] run function asset:mob/0378.red_knight_v3/tick/skill_select
