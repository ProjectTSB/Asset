...
#
#
# @within function asset:mob/0365.frestchika/tick/skill_branch
...
# ヘルス半減時はダッシュしながら弾を撒いてくる
    execute if score @s[tag=A5.Health.50Per] General.Mob.Tick matches 0..15 run function asset:mob/0365.frestchika/tick/base_move/skill/side_dash_shot/shoot/
...
# ブレーキ
    execute if score @s General.Mob.Tick matches 15 as @e[type=item_display,tag=A5.ModelRoot.Target,distance=..16,sort=nearest,limit=1] run function animated_java:tutankhamen/animations/dash_end/tween {to_frame: 0, duration: 5}
...
# ニュートラルに戻る
    execute if score @s General.Mob.Tick matches 30 as @e[type=item_display,tag=A5.ModelRoot.Target,distance=..16,sort=nearest,limit=1] run function animated_java:tutankhamen/animations/neutral/tween {to_frame: 0, duration: 5}
