...
# プレイヤーの方へ誘導する
    execute if score @s General.Mob.Tick matches ..15 run function asset:mob/0365.frestchika/tick/base_move/skill/front_dash_shot/move
...
# ブレーキ
    execute if score @s General.Mob.Tick matches 15 as @e[type=item_display,tag=A5.ModelRoot.Target,distance=..16,sort=nearest,limit=1] run function animated_java:frestchika/animations/dash_end/tween {to_frame: 0, duration: 5}
...
# ニュートラルポーズへ
    execute if score @s General.Mob.Tick matches 30 as @e[type=item_display,tag=A5.ModelRoot.Target,distance=..16,sort=nearest,limit=1] run function animated_java:frestchika/animations/neutral/tween {to_frame: 0, duration: 5}
