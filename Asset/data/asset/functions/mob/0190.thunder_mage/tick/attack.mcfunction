...
# ハード以上で50%の確率で偏差攻撃を行う
    ...
    execute if entity @s[tag=!Random] at @r[gamemode=!spectator,distance=..15] run function asset:mob/0190.thunder_mage/tick/summon
    execute if entity @s[tag=Random] if entity @p[gamemode=!spectator,distance=..15] run function asset:mob/0190.thunder_mage/tick/predict_attack
