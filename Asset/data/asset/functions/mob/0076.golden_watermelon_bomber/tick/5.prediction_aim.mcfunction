#> asset:mob/0076.golden_watermelon_bomber/tick/5.prediction_aim
#
# 偏差の処理
#
# @within function
...
# 偏差が遠くに行きすぎないようにスコアとは別に距離で制限をかける
    ...
    $execute if score $Recursive Temporary matches ..20 positioned ~$(X) ~ ~$(Z) if entity @s[distance=..30] unless entity @s[distance=..$(ReachableRange)] run function asset:mob/0076.golden_watermelon_bomber/tick/5.prediction_aim with storage asset:temp Predict
