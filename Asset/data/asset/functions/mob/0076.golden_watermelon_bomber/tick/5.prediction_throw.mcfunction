...
# 自身の弾速を設定
    scoreboard players set $Speed Temporary 15

# 自身の到達範囲を設定
    data modify storage asset:temp Predict.ReachableRange set value 1.5

# 偏差位置を割り出す
    execute at @p run function asset:mob/0076.golden_watermelon_bomber/tick/6.prediction_aim with storage asset:temp Predict
...
# 予測位置マーカーが存在しない場合、直接プレイヤーに投げる
    execute if entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] positioned ~ ~1000 ~ as @e[type=block_display,scores={MobID=77},distance=..5,limit=1] at @s positioned ~ ~-1000 ~ facing entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] eyes run tp @s ~ ~ ~ ~ ~-2
    execute unless entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] positioned ~ ~1000 ~ as @e[type=block_display,scores={MobID=77},distance=..5,limit=1] at @s positioned ~ ~-1000 ~ facing entity @p eyes run tp @s ~ ~ ~ ~ ~-2
