#> asset:mob/0440.dps_checker_fragile/register
#
# Mobのデータを指定
#
# @within function asset:mob/alias/440/register
...
# ID (int)
    data modify storage asset:mob ID set value 440
# Type (string) Wikiを参照
    data modify storage asset:mob Type set value "Enemy"
# 干渉可能か否か (boolean)
    data modify storage asset:mob Interferable set value true
# 名前 (TextComponentString) (オプション)
    data modify storage asset:mob Name set value '[{"text":"もろい訓練用の人形","color":"white"}]'
# Mobの説明文 (TextComponentString[]) (オプション)
    data modify storage asset:mob Lore set value ['{"text":"ちょっと壊れやすい。大事に扱うこと。","color":"white"}']
# 防具
