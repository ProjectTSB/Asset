...
# 予測位置マーカーが存在しない場合、直接プレイヤーを狙う
    ...
    execute if entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] anchored eyes positioned ^-0.25 ^ ^ run tp @e[type=marker,tag=58.Init,distance=..0.01,sort=nearest,limit=1] ~ ~ ~ facing entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] eyes
    execute unless entity @e[type=marker,tag=PredictionMarker,distance=..40,limit=1] anchored eyes positioned ^-0.25 ^ ^ run tp @e[type=marker,tag=58.Init,distance=..0.01,sort=nearest,limit=1] ~ ~ ~ facing entity @p[distance=..20] eyes
