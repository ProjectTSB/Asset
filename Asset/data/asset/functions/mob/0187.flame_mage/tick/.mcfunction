...
# 実行順の関係で強制実行を先に配置
    ...
    execute unless entity @s[scores={General.Mob.Tick=0..}] positioned ^ ^ ^10 if entity @p[gamemode=!spectator,distance=..10] run scoreboard players add @s General.Mob.Tick 1
...
# 射撃攻撃キャンセル
    execute unless entity @p[gamemode=!spectator,distance=..20] unless entity @s[scores={General.Mob.Tick=0..}] run scoreboard players set @s General.Mob.Tick -60
