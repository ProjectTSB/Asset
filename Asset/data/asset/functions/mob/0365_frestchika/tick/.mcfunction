#> asset:mob/0365_frestchika/tick/
#
# Mobのtick時の処理
#
...
# モデルを追従
    execute at @s run tp @e[type=item_display,tag=A5.ModelRoot.Target,distance=..16,sort=nearest,limit=1] ~ ~ ~ ~ ~
...
# 動作
    # イントロ
        execute if entity @s[tag=A5.Moveset.Intro] run function asset:mob/0365_frestchika/tick/intro/
    # 通常動作
        execute if entity @s[tag=A5.Moveset.BaseMove] run function asset:mob/0365_frestchika/tick/base_move/
    # フェイズ移行
        execute if entity @s[tag=A5.Moveset.Transition] run function asset:mob/0365_frestchika/tick/phase_transition/
