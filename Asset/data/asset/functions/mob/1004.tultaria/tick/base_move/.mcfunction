...
# 属性チェンジ
    ...
    # 光属性になった場合、ちょっと待ったあとにスカイボックスを展開しちゃおうかな
        execute if score @s[tag=RW.Mode.Light,tag=!RW.InAction] RW.Tick matches 30 at @e[type=marker,tag=RW.Marker.SpawnPoint,sort=nearest,limit=1] run summon item_display ~ ~ ~ {teleport_duration:10,Tags:["RW.SkyBox"],brightness:{sky:15,block:15},transformation:{left_rotation:[0.35f,0.14f,0.35f,0.85f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[-100f,-100f,-100f]},item:{id:"minecraft:leather_horse_armor",Count:1b,tag:{display:{color:16711680},CustomModelData:20381}}}
    # スカイボックスを回す
