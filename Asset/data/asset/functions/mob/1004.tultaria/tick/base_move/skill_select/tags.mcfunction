...
# デバッグ用処理: 技を名指しで指定
# TODO: ちゃんと消すんだよ！
    tag @s add RW.Skill.TransitionAttack2
    return fail

# 技を選択、属性ごとに違う
    # 火
        execute if entity @s[tag=RW.Mode.Fire] if data storage asset:context this.Skill{Get:0} run tag @s add RW.Skill.Meteor
        execute if entity @s[tag=RW.Mode.Fire] if data storage asset:context this.Skill{Get:1} run tag @s add RW.Skill.MoveShot
