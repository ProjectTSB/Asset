...
# モデルもこっちむく
    execute if score @s General.Mob.Tick matches 1..30 run function asset:mob/1004.tultaria/tick/skill/phase3/1.chase_and_shot/3.windup

# 設置
    execute if score @s General.Mob.Tick matches 1 run function asset:mob/1004.tultaria/tick/skill/phase3/1.chase_and_shot/2.place_marker
...
# マーカーをプレイヤーに追従させ、自身はそれを追う
    ...
    execute if score @s General.Mob.Tick matches ..35 facing entity @e[type=marker,tag=RW.ChaseMarker,distance=2..,sort=nearest,limit=1] feet positioned ^ ^ ^0.5 run function asset:mob/1004.tultaria/tick/move/tereport_no_facing
...
# 数Tickごとに射撃
    execute if score @s General.Mob.Tick matches 30 run function asset:mob/1004.tultaria/tick/skill/phase3/1.chase_and_shot/4.shoot

# ループする
    execute if score @s General.Mob.Tick matches 35 run scoreboard players set @s[scores={RW.LoopCount=..4}] General.Mob.Tick 25
...
# 小技出しましたのタグを付与
    execute if score @s General.Mob.Tick matches 60 run tag @s add RW.MiniSkillUsed
