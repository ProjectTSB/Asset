...
# 最初の構え
    execute if score @s RW.Tick matches 0 run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/windup

# 構えずターゲッティング(最初に実行)
    execute if score @s[scores={RW.LoopCount=0}] RW.Tick matches 10 run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/targeting_first
...
# 突き
    execute if score @s RW.Tick matches 20 run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/active
    # 突き部分のダメージ
        execute if score @s RW.Tick matches 21 positioned ^ ^ ^1 as @a[tag=!PlayerShouldInvulnerable,distance=..2] run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/stab/damage
    # 突きのエフェクト
        execute if score @s RW.Tick matches 0..25 positioned ^ ^ ^1 at @e[type=marker,tag=RW.ModelLocator.Sword,sort=nearest,limit=1] run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/stab/vfx
...
# 雷撃発動演出
    execute if score @s RW.Tick matches 33 run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/slash

# 雷撃
    # 初撃は強い
        execute if score @s RW.Tick matches 33 as @e[type=marker,tag=RW.ChainLightning.Common] at @s run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/chain_lightning/strong/
    # 持続は弱い
        execute if score @s RW.Tick matches 34..40 as @e[type=marker,tag=RW.ChainLightning.Common] at @s run function asset:mob/1004.tultaria/tick/skill/thunder/lightning_stab/chain_lightning/weak/
