...
# 自身のモデルにモーションを再生させる
    execute if score @s General.Mob.Tick matches 0 as @e[type=item_display,tag=RW.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:tultaria/animations/attack_magic_1_right/play
# 射撃
    execute if score @s General.Mob.Tick matches 3 run function asset:mob/1004.tultaria/tick/skill/water/homing_shot/shot/

# 自身のモデルにモーションを再生させる
    execute if score @s General.Mob.Tick matches 10 as @e[type=item_display,tag=RW.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:tultaria/animations/attack_magic_1_right/stop
    execute if score @s General.Mob.Tick matches 10 as @e[type=item_display,tag=RW.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:tultaria/animations/attack_magic_1_left/play
# 射撃
    execute if score @s General.Mob.Tick matches 13 run function asset:mob/1004.tultaria/tick/skill/water/homing_shot/shot/

# 自身のモデルにモーションを再生させる
    execute if score @s General.Mob.Tick matches 20 as @e[type=item_display,tag=RW.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:tultaria/animations/attack_magic_1_left/stop
    execute if score @s General.Mob.Tick matches 20 as @e[type=item_display,tag=RW.ModelRoot.Target,sort=nearest,limit=1] run function animated_java:tultaria/animations/attack_magic_1_right/play
# 射撃
