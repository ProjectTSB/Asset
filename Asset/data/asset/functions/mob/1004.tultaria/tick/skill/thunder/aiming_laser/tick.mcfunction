...
# 射撃
    # 偏差:ただしHard以下だと普通に狙ってくる
        # マーカー設置
            execute if predicate api:global_vars/difficulty/max/2_hard if score @s General.Mob.Tick matches 20 at @p[gamemode=!spectator,distance=..128] run summon marker ~ ~1.7 ~ {Tags:["RW.Marker.Aim"]}
