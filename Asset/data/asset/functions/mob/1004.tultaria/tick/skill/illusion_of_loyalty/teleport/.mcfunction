...
# モデルのターゲッティングを切る
    tag @e[type=item_display,tag=RX.ModelRoot,distance=..64] remove RX.ModelRoot.Target

# モデルにタグ付与
    execute at @s run tag @e[type=item_display,tag=RX.ModelRoot,sort=nearest,limit=1] add RX.ModelRoot.Target

# 拡散
    data modify storage lib: Argument.Bounds set value [[16d,16d],[0d,0.5d],[16d,16d]]
    execute at @e[type=marker,tag=RW.Marker.SpawnPoint,sort=nearest,limit=1] run function lib:spread_entity/

# 同じIDのモブが付近にいなければ、マーカーの位置にテレポート
