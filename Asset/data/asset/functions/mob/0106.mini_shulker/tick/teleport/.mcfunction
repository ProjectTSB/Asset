...
# マーカーを用意
    ...
    execute as @e[type=marker,tag=2Y.Append,distance=..15,limit=1] at @s align xyz run tp @s ~0.5 ~0.2399 ~0.5
# 壁の選定
    execute as @e[type=marker,tag=2Y.Append,distance=..15,limit=1] at @s run function asset:mob/0106.mini_shulker/tick/teleport/secure/
# 壁がなければ再試行
...
# リセット
    kill @e[type=marker,tag=2Y.Append,distance=..15,limit=1]
