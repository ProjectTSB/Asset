...
# プレイヤーを見つける
    execute store result score $7B.TargetID Temporary run data get storage asset:context this.TargetID 1
    execute at @s as @a[distance=..25] if score @s UserID = $7B.TargetID Temporary run tag @s add 7B.Target
