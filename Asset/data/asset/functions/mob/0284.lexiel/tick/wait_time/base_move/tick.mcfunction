...
# プレイヤーが周囲にいたらスキル選択
    execute if score @s[tag=!7W.InAction,tag=!7W.Move] 7W.Tick matches 25.. if entity @p[gamemode=!spectator,distance=..100] run function asset:mob/0284.lexiel/tick/skill_select
...
# 選択したスキル発動
    execute if entity @s[tag=7W.InAction] run function asset:mob/0284.lexiel/tick/skill_active
