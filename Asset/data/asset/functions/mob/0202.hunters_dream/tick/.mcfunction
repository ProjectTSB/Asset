...
# スコアが一定に達したら発動（プレイヤーが近くにいるとき）
    execute if score @s General.Mob.Tick matches 0 positioned ^ ^ ^5 if entity @p[gamemode=!spectator,distance=..8] at @s run function asset:mob/0202.hunters_dream/tick/3.skill_select
# スキル発動中
    execute if score @s General.Mob.Tick matches 0.. run function asset:mob/0202.hunters_dream/tick/4.skill_active
