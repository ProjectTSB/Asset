...
# 滑らかに回転
    execute as @e[type=wither_skeleton,tag=AV.Temp.This,distance=..64,sort=nearest,limit=1] at @s rotated ~ 0 positioned ^ ^ ^-2 facing entity @p[tag=AV.Temp.Target,distance=..66] feet rotated ~ 0 positioned ^ ^ ^-1 facing entity @s feet positioned as @s run tp @s ^ ^ ^ ~ 0
# 終了
