#> asset:mob/0046.clock_of_despair/tick/
#
# Mobのtick時の処理
#
# @within function asset:mob/alias/46/tick
...
# プレイヤーを引き寄せる
    execute as @a[gamemode=!spectator,distance=10..13] at @s facing entity @e[type=zombie,tag=this,distance=..13,sort=nearest,limit=1] eyes run tp @s ^ ^ ^0.4
    execute as @a[gamemode=!spectator,distance=13..20] at @s facing entity @e[type=zombie,tag=this,distance=..20,sort=nearest,limit=1] eyes run tp @s ^ ^ ^1.0
    execute as @a[gamemode=!spectator,distance=20..30] at @s facing entity @e[type=zombie,tag=this,distance=..30,sort=nearest,limit=1] eyes run tp @s ^ ^ ^1.5
    execute as @a[gamemode=!spectator,distance=30..100] at @s facing entity @e[type=zombie,tag=this,distance=..100,sort=nearest,limit=1] eyes positioned as @e[type=zombie,tag=this,distance=..100] rotated ~ 0 run tp @s ^ ^ ^2
