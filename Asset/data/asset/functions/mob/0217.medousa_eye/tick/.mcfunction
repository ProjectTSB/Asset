...
# スコアが60の時、向きを固定する
    execute if entity @s[scores={61.Tick=60}] run function asset:mob/0217.medousa_eye/tick/fix_direction

# スコアが61のとき、予備動作
    execute if entity @s[scores={61.Tick=61}] positioned ^ ^1.8 ^0.4 run function asset:mob/0217.medousa_eye/tick/vfx

# スコアが70以上で石化ビーム発射
    execute if entity @s[scores={61.Tick=70..}] run function asset:mob/0217.medousa_eye/tick/beam_shoot
