#> asset:mob/0349.louvert_soul_quake/tick/4.cast
#
# 発動
#
...
# ダメージを与える
    tag @a[tag=!PlayerShouldInvulnerable,distance=..1.5] add 9P.Landing
    execute positioned ~ ~1 ~ run tag @a[tag=!PlayerShouldInvulnerable,distance=..1.5] add 9P.Landing
    execute positioned ~ ~2 ~ run tag @a[tag=!PlayerShouldInvulnerable,distance=..1.5] add 9P.Landing
    execute as @a[tag=9P.Landing,distance=..16] at @s run function lib:damage/
# リセット
    function lib:damage/reset

# キル
    tag @a[tag=9P.Landing,distance=..16] remove 9P.Landing
    kill @s