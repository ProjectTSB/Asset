#> asset:mob/0456.gargo_ex_machina/tick/event/half_beam_sword/summon_laser_start
#
# 半面焼きビーム
#
...
# 演出
    ...
    execute as @e[type=area_effect_cloud,tag=CO.Aec.Target,distance=..10,limit=1] at @s positioned ^-2 ^ ^ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam_sword/summon_laser
    execute as @e[type=area_effect_cloud,tag=CO.Aec.Target,distance=..10,limit=1] at @s positioned ^2 ^ ^ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam_sword/summon_laser
