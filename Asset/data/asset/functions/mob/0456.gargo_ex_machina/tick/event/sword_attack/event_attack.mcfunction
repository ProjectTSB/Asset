...
# 攻撃
    # アニメーション再生
        ...
        execute if score @s CO.EventTimer matches 30 as @e[type=item_display,tag=CO.ModelRoot,sort=nearest,limit=1] run function animated_java:gargo_ex_machina/animations/sword_smash_attack/pause
    ...
    # 攻撃
        execute if score @s CO.EventTimer matches 10 positioned ^ ^ ^25 run function asset:mob/0456.gargo_ex_machina/tick/event/sword_attack/attack
    # 攻撃演出
        execute if score @s CO.EventTimer matches 12 positioned ^ ^ ^25 positioned as @e[type=item_display,tag=CO.ModelRoot,sort=nearest,limit=1] run playsound entity.breeze.shoot hostile @a ~ ~ ~ 2 0.9
        execute if score @s CO.EventTimer matches 12 positioned ^ ^ ^25 run function asset:mob/0456.gargo_ex_machina/tick/event/sword_attack/attack_effect_line.m {Dist:0}
        execute if score @s CO.EventTimer matches 14 positioned ^ ^ ^25 run function asset:mob/0456.gargo_ex_machina/tick/event/sword_attack/attack_effect_line.m {Dist:15}
        execute if score @s CO.EventTimer matches 16 positioned ^ ^ ^25 run function asset:mob/0456.gargo_ex_machina/tick/event/sword_attack/attack_effect_line.m {Dist:30}
