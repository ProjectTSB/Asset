...
# 溜め
    ...
    # 予告
        execute if entity @s[tag=CO.Skill.HalfBeam.IsRight] if score @s CO.EventTimer matches 35 positioned ^-25 ^ ^ rotated ~-30 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam/prediction.m {Tick:88}
        execute if entity @s[tag=CO.Skill.HalfBeam.IsRight] if score @s CO.EventTimer matches 55 positioned ^-25 ^0.05 ^ rotated ~30 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam/prediction.m {Tick:68}
        execute unless entity @s[tag=CO.Skill.HalfBeam.IsRight] if score @s CO.EventTimer matches 35 positioned ^25 ^ ^ rotated ~30 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam/prediction.m {Tick:88}
        execute unless entity @s[tag=CO.Skill.HalfBeam.IsRight] if score @s CO.EventTimer matches 55 positioned ^25 ^0.05 ^ rotated ~-30 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/half_beam/prediction.m {Tick:68}
    # 効果音
