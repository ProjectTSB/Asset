...
# 構え
    ...
    # 召喚
        ...
        # 床召喚位置が配置されていない場合、ランダム配置
            execute if score @s CO.EventTimer matches 22 unless entity @e[type=area_effect_cloud,tag=CO.Aec.FieldModify,distance=..80] positioned as @e[type=marker,tag=CO.CenterPosition,distance=..80,sort=nearest,limit=1] positioned ~ ~-0.499 ~ rotated 0 0 run function asset:mob/0456.gargo_ex_machina/tick/event/field_modify/summon_field_start
        execute if score @s CO.EventTimer matches 22 as @e[type=area_effect_cloud,tag=CO.Aec.FieldModify,distance=..80] at @s run function asset:mob/0456.gargo_ex_machina/tick/event/field_modify/summon_field
