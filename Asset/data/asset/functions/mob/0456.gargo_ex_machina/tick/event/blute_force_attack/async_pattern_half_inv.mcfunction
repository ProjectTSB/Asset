#> asset:mob/0456.gargo_ex_machina/tick/event/blute_force_attack/async_pattern_half_inv
#
# ブルートフォースアタック
#
...
# 2回目
    ...
    # YEETER
        execute if score @s CO.EventTimer.BluteForce matches 201 rotated ~180 ~ positioned ^25 ^ ^28 rotated ~180 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/blute_force_attack/summon_yeeter_drone.m {IsRotate:"false",IsRotateInverse:"false"}
        execute if score @s CO.EventTimer.BluteForce matches 201 rotated ~180 ~ positioned ^12 ^ ^28 rotated ~180 ~ run function asset:mob/0456.gargo_ex_machina/tick/event/blute_force_attack/summon_yeeter_drone.m {IsRotate:"false",IsRotateInverse:"false"}
