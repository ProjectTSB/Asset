#> asset:mob/0341.louvert/tick/animation/11_2_set_bomb/
#
# 汎用魔法実行アニメーション
#
...
# 演出
    execute if score @s 9H.AnimationTick matches 41 positioned ~ ~1 ~ run function asset:mob/0341.louvert/tick/animation/11_2_set_bomb/vfx

# ボム 発動 （ハードだと2倍）
    execute if score @s 9H.AnimationTick matches 41 at @a[gamemode=survival,tag=!PlayerShouldInvulnerable,distance=..32,sort=random,limit=3] positioned ~ ~0.5 ~ run function asset:mob/0341.louvert/tick/animation/11_2_set_bomb/summon
...
# アニメーション終了処理
    execute if score @s 9H.AnimationTick matches 56 run scoreboard players set @s 9H.AnimationNum 11
    execute if score @s 9H.AnimationTick matches 56 run scoreboard players set @s 9H.AnimationTick 0