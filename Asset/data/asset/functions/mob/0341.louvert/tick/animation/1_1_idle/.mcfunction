...
# アニメーション再生
    execute if predicate api:global_vars/difficulty/max/2_hard if score @s 9H.AnimationTick matches 41 as @e[type=item_display,tag=9H.Root.This,distance=..100] run function animated_java:louvert/animations/01_idle/play
...
# アニメーション終了処理
    execute if predicate api:global_vars/difficulty/max/2_hard if score @s 9H.AnimationTick matches 81 run function asset:mob/0341.louvert/tick/animation/1_1_idle/end
