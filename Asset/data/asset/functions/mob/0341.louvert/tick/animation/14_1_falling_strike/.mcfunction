...
# ソウルクエイク発動3
    execute if score @s 9H.AnimationTick matches 36 run function asset:mob/0341.louvert/tick/animation/14_1_falling_strike/third

# ソウルクエイク発動4
    execute if score @s 9H.AnimationTick matches 39 run function asset:mob/0341.louvert/tick/animation/14_1_falling_strike/forth

# 追尾ソウルクエイク発動
    tag @s add 9H.Temp.This
    execute if score @s 9H.AnimationTick matches 39 as @a[gamemode=survival,tag=!PlayerShouldInvulnerable,distance=..100,limit=4] as @e[tag=9H.Temp.This,distance=..100] run function asset:mob/0341.louvert/tick/animation/14_1_falling_strike/hard
