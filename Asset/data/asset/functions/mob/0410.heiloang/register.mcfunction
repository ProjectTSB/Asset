...
# ダメージ
    ...
    # なぎはらい火炎放射
        data modify storage asset:mob Field.Damage.Sweep set value 55.0f
    # テンペスト：落下ダメージは含まない
        data modify storage asset:mob Field.Damage.Tempest set value 58.0f
    # テルツェット・サクセッション
        # ダイブ
            data modify storage asset:mob Field.Damage.TerSuccStart set value 65.0f
        # 連続ダイブ
            data modify storage asset:mob Field.Damage.TerSuccDive set value 60.0f
    # テルツェット・ディザスター
        # 外周の雷
            data modify storage asset:mob Field.Damage.TerDisBlitz set value 65.0f
    # テルツェット・アライニング
        # 雷
            data modify storage asset:mob Field.Damage.TerAligBlitz set value 55.0f
        # ヘヴンリースパイク
            data modify storage asset:mob Field.Damage.TerAligIce set value 45.0f
    # テルツェット・プルガトリオ
        # ダイブはTerSuccStartを使用
        # ロックオン雷
            data modify storage asset:mob Field.Damage.TerPurgThunder set value 65.0f
    # テルツェット・ラグナレク
        # 直線雷はTerAlihBlitzを使用
        # 回転雷はBlitzSpinを使用
        # テンペストはTempestを使用
    ...
    # P4：パワーチャージ段階に応じてダメージ増加、目安は1段階につき+10%くらい
        ...
        # パワーブレス
            # 直撃
                data modify storage asset:mob Field.Damage.PowerBreath0 set value 65.0f
                data modify storage asset:mob Field.Damage.PowerBreath1 set value 70.0f
                data modify storage asset:mob Field.Damage.PowerBreath2 set value 80.0f
            # 衝撃波
                data modify storage asset:mob Field.Damage.PbImpact0 set value 45.0f
                data modify storage asset:mob Field.Damage.PbImpact1 set value 50.0f
