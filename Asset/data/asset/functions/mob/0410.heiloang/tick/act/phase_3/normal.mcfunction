#> asset:mob/0410.heiloang/tick/act/phase_3/normal
#
# フェーズ3
#
...
# 待機
    execute if score @s BE.ActCount matches 4 as @e[type=item_display,tag=BE.ModelRoot,sort=nearest,limit=1] run scoreboard players set @s BE.Idle.Count 30

# テルツェット・ディザスター
    execute if score @s BE.ActCount matches 5 run tag @s add BE.Skill.Ter.Dis.Start

# ソウルオファリング
    execute if score @s BE.ActCount matches 6 run tag @s add BE.Skill.Offering

# フェーズ4に移行
    execute if score @s BE.ActCount matches 7 run function asset:mob/0410.heiloang/tick/act/phase_3/change_phase
