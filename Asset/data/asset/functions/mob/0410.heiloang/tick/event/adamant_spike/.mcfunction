...
# 詠唱：2回
    ...
    # ハード以上：ターゲット決定
        execute if score @s BE.EventTimer matches 1 if predicate api:global_vars/difficulty/min/hard as @a[distance=..40,sort=random,limit=1] run tag @s add BE.MainTarget
    # 氷設置位置予告
