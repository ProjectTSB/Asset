...
# 発動
    ...
    # 攻撃
        ...
        # ノーマル以上：プレイヤー位置に落雷設置
            execute if score @s BE.EventTimer matches 112 if predicate api:global_vars/difficulty/min/2_hard positioned as @a[tag=!PlayerShouldInvulnerable,distance=..80] run function asset:mob/0410.heiloang/tick/event/richt_blitzen/summon_point
            execute if score @s BE.EventTimer matches 137 if predicate api:global_vars/difficulty/min/2_hard positioned as @a[tag=!PlayerShouldInvulnerable,distance=..80] run function asset:mob/0410.heiloang/tick/event/richt_blitzen/summon_point
            execute if score @s BE.EventTimer matches 162 if predicate api:global_vars/difficulty/min/2_hard positioned as @a[tag=!PlayerShouldInvulnerable,distance=..80] run function asset:mob/0410.heiloang/tick/event/richt_blitzen/summon_point
        # ハード以上：回転する雷を設置
