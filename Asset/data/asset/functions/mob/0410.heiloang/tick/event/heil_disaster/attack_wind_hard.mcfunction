#> asset:mob/0410.heiloang/tick/event/heil_disaster/attack_wind_hard
#
# ヘイルディザスター
#
...
# 弾召喚
    data modify storage api: Argument.ID set value 2185
    data modify storage api: Argument.FieldOverride.Rotation set from entity @s Rotation
    execute store result storage api: Argument.FieldOverride.MobUUID int 1 run scoreboard players get @s MobUUID
    data modify storage api: Argument.FieldOverride.Damage set from storage asset:context this.Damage.WindBurstHard
