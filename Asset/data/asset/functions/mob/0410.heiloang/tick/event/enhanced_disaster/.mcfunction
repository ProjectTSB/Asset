...
# 詠唱：2回
    ...
    # 属性予告
        execute if score @s BE.EventTimer matches 25..45 run function asset:mob/0410.heiloang/tick/event/enhanced_disaster/particle_element.m {Count:1}
    ...
    # 攻撃位置予告
        ...
        # execute if score @s BE.EventTimer matches 43 at @e[type=marker,tag=BE.CenterPosition] run function asset:mob/0410.heiloang/tick/event/enhanced_disaster/summon_circle
        # execute if score @s BE.EventTimer matches 63 at @e[type=marker,tag=BE.CenterPosition] run function asset:mob/0410.heiloang/tick/event/enhanced_disaster/summon_circle
