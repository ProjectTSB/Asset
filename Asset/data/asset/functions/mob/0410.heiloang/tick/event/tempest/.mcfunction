...
# 詠唱：3回
    ...
    # ターゲット決定
        execute if score @s BE.EventTimer matches 1 as @a[tag=!PlayerShouldInvulnerable,distance=..60,sort=random,limit=5] run tag @s add BE.AttackTarget
    # 攻撃範囲予告
        execute if score @s BE.EventTimer matches 10 at @a[tag=BE.AttackTarget,tag=!PlayerShouldInvulnerable,distance=..80] rotated ~ 0 run function asset:mob/0410.heiloang/tick/event/tempest/attack
        execute if score @s BE.EventTimer matches 10..140 as @a[tag=BE.AttackTarget,tag=!PlayerShouldInvulnerable,distance=..80] at @s rotated ~ 0 positioned ~ ~0.1 ~ run function asset:mob/0410.heiloang/tick/event/tempest/particle_attack_area
...
# 発動
    ...
    # 攻撃
        # execute if score @s BE.EventTimer matches 145 at @a[tag=BE.AttackTarget,tag=!PlayerShouldInvulnerable] rotated ~ 0 run function asset:mob/0410.heiloang/tick/event/tempest/summon_attack_pos
