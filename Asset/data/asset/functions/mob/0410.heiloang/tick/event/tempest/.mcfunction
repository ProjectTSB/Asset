...
# 詠唱：3回
    ...
    # 攻撃範囲予告
        execute if score @s BE.EventTimer matches 10 at @a[tag=BE.AttackTarget] rotated ~ 0 run function asset:mob/0410.heiloang/tick/event/tempest/summon_attack_pos
        execute if score @s BE.EventTimer matches 10..135 as @a[tag=BE.AttackTarget] at @s rotated ~ 0 positioned ~ ~0.1 ~ run function asset:mob/0410.heiloang/tick/event/tempest/particle_attack_area
        execute if score @s BE.EventTimer matches 10..135 run tag @e[type=item_display,tag=BE.Temp.MoveEnd] remove BE.Temp.MoveEnd
# 発動
    # アニメーション再生
