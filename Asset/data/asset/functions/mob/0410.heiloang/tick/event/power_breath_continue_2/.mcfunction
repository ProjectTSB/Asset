#> asset:mob/0410.heiloang/tick/event/power_breath_continue_2/
#
# パワーブレス
#
...
# 発動
    # アニメーション再生
        execute if score @s BE.EventTimer matches 1 as @e[type=item_display,tag=BE.ModelRoot,sort=nearest,limit=1] run function asset:mob/0410.heiloang/tick/animated_java/play/10_3_power_breath_continue_2
    # 攻撃位置決定
...
# 終了
    execute if score @s BE.EventTimer matches 26.. run function asset:mob/0410.heiloang/tick/event/power_breath_continue_2/end
