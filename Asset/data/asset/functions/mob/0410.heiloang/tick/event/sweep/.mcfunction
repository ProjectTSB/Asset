...
# 発動
    ...
    # 演出
        execute if score @s BE.EventTimer matches 45 as @e[type=item_display,tag=BE.ModelRoot,sort=nearest,limit=1] at @s on passengers if entity @s[tag=aj.data] run function asset:mob/0410.heiloang/tick/event/sweep/text_start with entity @s data.locators.beam_start
        execute if score @s BE.EventTimer matches 45..105 as @e[type=item_display,tag=BE.ModelRoot,sort=nearest,limit=1] at @s on passengers if entity @s[tag=aj.data] run function asset:mob/0410.heiloang/tick/event/sweep/text with entity @s data.locators.beam_start
    # 角度
