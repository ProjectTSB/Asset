...
# 本体の向きを変える
    execute facing entity @p[gamemode=!spectator,distance=..128] eyes rotated ~ ~-10 run tp @s ~ ~ ~ ~ ~
...
# 攻撃箇所表示
    ...
    execute rotated as @s rotated ~ 0 positioned as @p[gamemode=!spectator,distance=..128] positioned ^ ^ ^-30 positioned ~ ~0.01 ~ run function asset:mob/0213.terrible_sonic_bomber/tick/weapons/set_attack_position_display.m with storage asset:temp args
