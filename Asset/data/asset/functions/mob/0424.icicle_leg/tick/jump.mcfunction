#> asset:mob/0424.icicle_leg/tick/jump
#
#
#
...
# こっちに対してMotionで飛ぶ 近いやつを優先
    ...
    execute if entity @a[gamemode=!spectator,distance=7..14] run data modify storage lib: Argument.VectorMagnitude set value 1.5
    execute if entity @a[gamemode=!spectator,distance=..7] run data modify storage lib: Argument.VectorMagnitude set value 0.7
