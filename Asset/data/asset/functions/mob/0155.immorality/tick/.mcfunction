...
# 発砲準備で向きと位置を固定し、射撃回数を計算する
    execute if entity @s[scores={4B.Tick=0}] run function asset:mob/0155.immorality/tick/shot/ready

# 発砲
    execute if entity @p[distance=..20] if entity @s[scores={4B.Tick=20..}] anchored eyes positioned ^-0.3 ^ ^1 run function asset:mob/0155.immorality/tick/shot/interval
