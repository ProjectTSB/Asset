...
# 周囲にプレイヤーがいなければリセット
    execute if entity @s[scores={A9.Tick=0}] unless entity @p[distance=..20] run function asset:mob/0369.prominence_spellbook/tick/reset
...
# execute summonを使って実行する
    execute if entity @s[scores={A9.Tick=10..}] anchored eyes positioned ^ ^ ^0.2 summon marker run function asset:mob/0369.prominence_spellbook/tick/summon

# リセット
    execute if entity @s[scores={A9.Tick=10..}] run function asset:mob/0369.prominence_spellbook/tick/reset
