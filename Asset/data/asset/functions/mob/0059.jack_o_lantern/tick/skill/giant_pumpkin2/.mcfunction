...
# ハード以上なら2面ずつ転がす
    ...
    # 0tick目に配列を初期化し、2回転がす
        ...
        execute if entity @s[scores={1N.Tick=0}] if predicate api:global_vars/difficulty/max/normal run function asset:mob/0059.jack_o_lantern/tick/skill/giant_pumpkin2/select_cross
        execute if entity @s[scores={1N.Tick=50}] if predicate api:global_vars/difficulty/max/normal run function asset:mob/0059.jack_o_lantern/tick/skill/giant_pumpkin2/select_cross

    # ハード
        execute if entity @s[scores={1N.Tick=0..25}] if predicate api:global_vars/difficulty/min/hard run function asset:mob/0059.jack_o_lantern/tick/skill/giant_pumpkin2/select_cross_hard
...
# 直線に並んだやつを転がす
    execute if entity @s[scores={1N.Tick=130}] run function asset:mob/0059.jack_o_lantern/tick/skill/giant_pumpkin2/select_line
