...
# 体力が半分以下であることを検知して3way弾にする
    execute if entity @s[tag=2W.HPless50Per] facing entity @p[tag=!PlayerShouldInvulnerable] feet rotated ~20 ~ run function asset:mob/0104.mad_scientist/tick/addition_throw
    execute if entity @s[tag=2W.HPless50Per] facing entity @p[tag=!PlayerShouldInvulnerable] feet rotated ~-20 ~ run function asset:mob/0104.mad_scientist/tick/addition_throw
