...
# CannotMakeScaffoldTick:0かつ空中なら、足場を生成
    execute if data storage asset:context this{CannotMakeScaffoldTick:0} if block ~ ~-1 ~ #lib:no_collision_without_fluid if block ~ ~-2 ~ #lib:no_collision_without_fluid if block ~ ~-3 ~ #lib:no_collision_without_fluid run function asset:mob/0331.aurora_sorcerer/tick/make_scaffold/
